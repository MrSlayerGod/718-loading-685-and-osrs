...
public final class GeneralRequirementMap {
	...
	public static void modify(GeneralRequirementMap config) {
		if (config.id == 979 //arcane neck
				|| config.id == 1030 //ring of vigour
				|| config.id == 1156 //demon horn neck
				|| config.id >= 991 && config.id <= 998) { //chaotics + shield
		//	config.values.put((long) 1072, config.getIntValue(1072)*5);
			return;
		}
	}
	...
	public static final GeneralRequirementMap getMap(int scriptId, boolean loadNewFormat) {
		...
		script = new GeneralRequirementMap(scriptId);
		if (data != null)
	...
	private GeneralRequirementMap(int id) {
		this.id = id;
	}
}

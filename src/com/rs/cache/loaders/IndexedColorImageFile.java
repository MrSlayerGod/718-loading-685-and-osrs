...
public final class IndexedColorImageFile {
	...
	public IndexedColorImageFile(BufferedImage... images) {
        ...
        public void decodeArchive(Store cache, int idx, int archiveId, int fileId) {
			...
			if (count == 65535) {
				newFormat = true;
				stream.setOffset(data.length - 4);
            ...
            stream.setOffset(data.length - (newFormat ? 12 : 7) - images.length * 8);
            ...
            stream.setOffset(data.length - (newFormat ? 12 : 7) - images.length * 8 - (palleteLength - 1) * 3);
	...
	public int getPalleteIndex(int rgb) {
		...
		//if ((c.getRed() == 255 && c.getGreen() == 255 && c.getBlue() == 255)
			...
			if(pallete.length == Integer.MAX_VALUE - 1) { //256
	...
	public void trim(int size) {
		BufferedImage[] newImages = Arrays.copyOf(images, size);
		images = newImages;
		pallete = null;
		pixelsIndexes = null;
		alpha = null;

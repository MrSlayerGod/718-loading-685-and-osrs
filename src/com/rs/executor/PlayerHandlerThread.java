...
public final class PlayerHandlerThread extends Thread {
	...
	private final void processPlayerSave(SaveDetails details) {
		...
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFileTransmitInit(details.username, details.data.length).trim());
		for (int i = 0; i < parts.length; i++)
			LoginClientChannelManager.sendReliablePacket(parts[i].trim());
	}
	...
	private final boolean processLogoutPlayer(Player player) {
		...
		if (data != null && data.length > 0) {
			...
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodePlayerFileTransmitInit(player.getUsername(), data.length).trim());
			for (int i = 0; i < parts.length; i++)
				LoginClientChannelManager.sendReliablePacket(parts[i].trim());
		}
		LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodeLogout(player.getUsername()).trim());
		return true;
	}
...
//		}
		if (details.step == 0) { // send request to login server
			...
			if (Utils.currentTimeMillis() > details.waitTime) {
				...
				LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodeLogout(details.username).trim());
	...
	private final void initPlayer(Player player, LoginDetails details) {
		player.init(details.session, details.lobby, details.username, details.displayName, details.macAddress, details.email, details.rights, details.gameMode, details.messageIcon, details.masterLogin, details.donator, details.support, details.gfxDesigner, details.youtuber, details.muted, details.lastVote, details.displayMode, details.screenWidth, details.screenHeight, details.machineInfo, new IsaacKeyPair(details.sessionKeys));
		if (details.lobby) {
	 ...
	 */
	public static final void handleLoginResponse(int sessionid, String sessionuser, int status, int file_length, int rights, int gameMode, boolean masterLogin, int donator, boolean support, boolean gfxDesigner, boolean youtuber, int messageIcon, boolean muted, long lastVote, String displayName, String email) {
		synchronized (lock) {
			LoginDetails details = waitingLoginSessions.get(sessionid);
			if (details == null && status == 2) {
				// we did not actually log in
				LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodeLogout(sessionuser).trim());

...
public final class CastleWars {
	...
	public static final int LOBBY_TIME = 1, GAME_TIME = 10; //time
	...
	public static final Map<Integer, Integer> rewards = new HashMap<Integer, Integer>();
	public static ArrayList<Integer> rewards_int;
	...
	public static void init() {
		...
		rewards_int = new ArrayList<Integer>();
		rewards.values().forEach(rewards_int::add);
	}
	...
	public static void joinPortal(Player player, int team) {
		...
		player.getMusicsManager().playMusic(249); // //fixed
					...
					for (Player player : playing[i].toArray(new Player[playing[i].size()])) {
						...
						if (winner != -1) {
							...
							if (winner == -2) {
								...
								player.getInventory().addItem(CW_TICKET, 6 * (Settings.DOUBLE_MINIGAME_ENABLED ? 2 : 1));
								/**if (Utils.random(500) == 0)
									dropSet(player);*/
								Item item = new Item(MinigameBox.ID, 3);
								...
								player.getInventory().addItem(CW_TICKET, 9 * (Settings.DOUBLE_MINIGAME_ENABLED ? 2 : 1));
							/*	if (Utils.random(200) == 0)
									dropSet(player);*/
								Item item = new Item(MinigameBox.ID, 5);
								player.getPackets().sendGameMessage(item.getName() +" x"+item.getAmount()+" has been added to your bank.");
								player.getBank().addItem(item.getId(), item.getAmount(), false);
							} else {
							...
							player.getInventory().addItem(CW_TICKET, 3 * (Settings.DOUBLE_MINIGAME_ENABLED ? 2 : 1));
						}
					}
			}
		}, 6);
	...
	public static final int[] PIECES = {21472, 21473, 21474, 21475, 21476};
	...
	private static void dropSet(Player player) {
		...
		World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(piece).getName() + "<col=ff8c38> from <col=cc33ff>castle wars<col=ff8c38>!", 1);
			}
	...
	public static void refreshTimeLeft(Player player) {
		player.getVarsManager().sendVar(380, playingGame == null ? 0 : playingGame.minutesLeft - (player.getControlerManager().getControler() instanceof CastleWarsPlaying ? LOBBY_TIME : 0));
	}
	...
	public static void refreshAllPlayersTime() {
		...
		for (int i = 0; i < playing.length; i++)
			for (Player player : playing[i]) {
				player.getMusicsManager().playMusic(i == ZAMORAK ? 759 : 247); //fixed
				refreshTimeLeft(player);
			}
	}
	...
	private static class PlayingGame extends TimerTask {
		...
		public void run() {
			try {
				minutesLeft--;
				if (minutesLeft ==/* 5*/LOBBY_TIME) {
					endGame(score[SARADOMIN] == score[ZAMORAK] ? -2 : score[SARADOMIN] > score[ZAMORAK] ? SARADOMIN : ZAMORAK);
					World.sendNews(null,  "<col=9999ff>A castle wars game just finished. Results: Zamorak - "+score[ZAMORAK]+" / Saradomin - "+score[SARADOMIN], 1);
					reset();
				} else if (minutesLeft == 0) {
					minutesLeft = GAME_TIME + LOBBY_TIME;
					startGame();
				} else if (minutesLeft > /*6*/LOBBY_TIME) // adds ppl waiting on lobby
					startGame();
				refreshAllPlayersTime();
			} catch (Throwable e) {
				Logger.handle(e);
	...
	public static void interactCastleWarsTicketExchange(Player player, int componentId, boolean buy) {
		...
		if (buy) {
			...
			if (!player.getInventory().addItem(item))
				return;
			player.getCollectionLog().add(CategoryType.MINIGAMES, "Castle Wars", item);

...
public final class WarControler extends Controller {
	...
	public boolean sendDeath() {
		player.lock(8);
		player.stopAll();
		if (player.getFamiliar() != null && clanWars.get(Rules.ITEMS_LOST))
	...
	public void sendInterfaces() {
		player.getInterfaceManager().setOverlay(265, false);
	}
	...
	public boolean keepCombating(Entity victim) {
		if (player.getCombatDefinitions().getType() == Combat.MAGIC_TYPE) {
		...
		if (player.getCombatDefinitions().getType() == Combat.RANGE_TYPE && clanWars.get(Rules.NO_RANGE)) {
			player.getPackets().sendGameMessage("Ranged combat is not allowed during this war!");
			return false;
		}
		if (player.getCombatDefinitions().getType() == Combat.MELEE_TYPE && clanWars.get(Rules.NO_MELEE)) {

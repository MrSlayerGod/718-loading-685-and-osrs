...
public class Helper {
	...
	public static final long DURATION = 10 * 60 * 1000; //original 20min
	...
	public static void sendHome(Player player) {
		reset(player);
		player.useStairs(-1, Helper.EXIT, 0, 2);
	...
	public static void spawn(StealingCreationGame game, Player player, boolean team) {
		...
		// after teleing
		player.getMusicsManager().playMusic(620); //fixed
	}
	...
	public static void awardPoints(Player player, Score personal, int winner) {
		int points = calculateReward(personal, winner) * 3 * (Settings.DOUBLE_MINIGAME_ENABLED ? 2 : 1);
		if (points > 0) {
		...
		/*	if ((personal.getTeam() ? 2 : 1) == winner && Utils.random(400) == 0)
				dropSet(player);*/
			Item item = new Item(MinigameBox.ID, ((personal.getTeam() ? 2 : 1) == winner ? 2 : 1));
	...
	private static void dropSet(Player player) {
		...
		World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(piece).getName() + "<col=ff8c38> from <col=cc33ff>stealing creation<col=ff8c38>!", 1);
	}
	...
	public static void displayKiln(final Player player) {
		...
		player.getPackets().sendExecuteScript(KILN_SCRIPT_BASE + 0); // + tabID
		int quality = 0;

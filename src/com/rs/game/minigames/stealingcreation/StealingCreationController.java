...
public class StealingCreationController extends Controller {
	...
	public boolean canPlayerOption3(final Player target) {
		...
		if (PICKPOCK_DELAY != null && PICKPOCK_DELAY > Utils.currentTimeMillis()) {
			...
			return false;
		} else if (!(target.getControlerManager().getControler() instanceof StealingCreationController) || target.getTemporaryAttributtes().get("in_kiln") != null && (Long) target.getTemporaryAttributtes().get("in_kiln") >= Utils.currentTimeMillis()) {

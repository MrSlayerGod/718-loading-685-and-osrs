...
import com.rs.game.player.content.Consumables.Food;
...
public class DuelArena extends Controller {
						...
						if (stage != DuelStage.DONE) {
							...
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									...
									if (DuelControler.isAtDuelArena(oldTarget))
	...
	public void addItem(int slot, int amount) {
		synchronized (this) {
			...
			synchronized (target.getControlerManager().getControler()) {
				...
				if(target.getInventory().getFreeSlots() < slotsRequired(item)) {
					player.sendMessage("Your opponent doesn't have enough inventory space.");
					return;
				}
	...
	public int slotsRequired(Item stakedItem) {
		ArrayList<Item> items = new ArrayList<>();
		for(Item item : player.getDuelRules().getStake().getItems()) {
			if(item==null)continue;
			items.add(item);
		}
		for(Item item : target.getDuelRules().getStake().getItems()) {
			if(item==null)continue;
			if(item.getDefinitions().stackable != 0 && items.stream().noneMatch(i -> i.getId() == item.getId()))
				items.add(item);
			else if(item.getDefinitions().stackable == 0)
				items.add(item);
		}
		if(stakedItem.getDefinitions().stackable != 0 && items.stream().noneMatch(i->i.getId() == stakedItem.getId()))
			items.add(stakedItem);
		else if(stakedItem.getDefinitions().stackable == 0)
			for(int i = 0; i < stakedItem.getAmount(); i++)
				items.add(new Item(stakedItem.getId(), 1));
		return items.size();
	}
	...
	public void cancelAccepted() {
		...
		if (player.getTemporaryAttributtes().get("acceptedDuel") == Boolean.TRUE) {
			player.getTemporaryAttributtes().put("acceptedDuel", false);
			canceled = true;
		}
	...
	public void endDuel(final Player victor, final Player loser, boolean isDraw) {
		...
		if (rules != null && !rules.hasRewardGiven()) {
			...
			if (!isDraw/* && victor.getDuelRules().getStake().getUsedSlots() != 0 && loser.getDuelRules().getStake().getUsedSlots() != 0*/)
				victor.setWonStackedDuel();
			...
			for (Item item : loser.getDuelRules().getStake().getItems()) {
				...
				if (isDraw)
					...
					victor.getInventory().addItemDrop(item.getId(), item.getAmount());
					Bot.sendLog(Bot.TRADE_STACK_CANNEL, "[type=DUEL][name="+loser.getUsername()+"][to="+victor.getUsername()+"][item="+item.getName()+"("+item.getId()+")x"+Utils.getFormattedNumber(item.getAmount())+"]");
				}
			}
		...
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				if (DuelControler.isAtDuelArena(loser))
					loser.getControlerManager().startControler("DuelControler");
				if (DuelControler.isAtDuelArena(victor))
		...
		for (int i = 10; i < 23; i++) {
			if (i == 13 || i == 15) {//shield or wep
				if (player.getEquipment().hasTwoHandedWeapon()) {
					if (!ButtonHandler.sendRemove(target, 3)) {
						return false;
					}
				}
			}
			if (player.getDuelRules().getRule(i)) {
				slot = i - 10;
				if (!ButtonHandler.sendRemove(player, slot)) {
					return false;
				}
			}
		}
	...
	public boolean keepCombating(Entity victim) {
		...
		if (player.getCombatDefinitions().getType() == Combat.MAGIC_TYPE && rules.getRule(2) && isDueling) {
			player.getPackets().sendGameMessage("You cannot use Magic in this duel!");
			return false;
		} else if (player.getCombatDefinitions().getType() == Combat.RANGE_TYPE && rules.getRule(0) && isDueling) {
			player.getPackets().sendGameMessage("You cannot use Range in this duel!");
			return false;
		} else if (player.getCombatDefinitions().getType() == Combat.MELEE_TYPE && rules.getRule(1) && isDueling) {
	...
	public boolean canEquip(int slotId, int itemId) {
		...
		if (slotId == Equipment.SLOT_AURA) {	
			player.getPackets().sendGameMessage("You can't equip " + ItemConfig.forID(itemId).getName().toLowerCase() + " during this duel.");
			return false;
		}
		if (isDueling) {

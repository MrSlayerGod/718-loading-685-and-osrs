...
public class DuelRules {
	...
	public boolean canAccept(ItemsContainer<Item> stake) {
		...
		if (player.getEquipment().getAuraId() != -1) {
			player.getPackets().sendGameMessage("Auras have been disabled during this duel!");
			getTarget().getPackets().sendGameMessage("Auras have been disabled during this duel!");
			return false;
		}
		int count = 0;
		Item item;

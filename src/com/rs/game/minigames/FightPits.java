...
public final class FightPits {
	...
	public static void enterArena(Player player) {
		...
		player.getMusicsManager().forcePlayMusic(MUSICS[Utils.random(MUSICS.length)]); //fixed
		arena.add(player);
	}
		...
		synchronized (lock) {
			...
			if (type == 0) {
				...
				if (player.hasSkull() && player.getSkullId() == 1) {// if has
					...
					int tokkul = (lobby.size() + arena.size()) * 100 * 20; //boosted tokkul from fight pits
					...
					if (Utils.random(100) == 0 && !player.getInventory().addItem(6571, 1) && type == 1) {
						World.addGroundItem(new Item(6571, 1), new WorldTile(4585, 5076, 0), player, true, 180);
						World.sendNews(player, player.getDisplayName() + " has received uncut onyx from fight pits!", 1);
					}
				...
				if (type == 1) {
					...
					player.useStairs(-1, new WorldTile(new WorldTile(4592, 5073, 0), 2), 0, 2);
			}
			refreshFoes();
			checkPlayersAmmount();
	...
	public static final int[] PIECES = {21467, 21468, 21469, 21470, 21471};
	...
	private static void dropSet(Player player) {
		...
		World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(piece).getName() + "<col=ff8c38> from <col=cc33ff>fight pits<col=ff8c38>!", 1);
	}
	...
	public static void setChampion() {
		...
		if (currentChampion == null || !currentChampion.equals(champion.getDisplayName()))
			World.sendNews(champion, "<col=9999ff>"+champion.getDisplayName() + " is now the fight pits champion!", 1);

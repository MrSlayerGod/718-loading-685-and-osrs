...
public class PuroPuro extends Controller {
	...
	public static final Item[] REWARD =
	...
	public static void initPuroImplings() {
		for (int i = 0; i < 5; i++) {
			for (int index = 0; index <= FlyingEntities.KINGLY_IMPLING.ordinal(); index++) {
				/*if (i > 2) {
	...
	public static void confirmPuroSelection(Player player) {
		...
		if (player.getInventory().addItemDrop(exchangedItem.getId(), exchangedItem.getAmount())) {
			...
			player.getCollectionLog().add(CategoryType.MINIGAMES, "Puro-Puro", new Item(exchangedItem.getId(),exchangedItem.getAmount()));

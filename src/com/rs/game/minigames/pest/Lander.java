...
public class Lander {
	...
	private static final int TIME = 30;//150;
	...
	public class LobbyTimer extends TimerTask {
		...
		public void run() {
			try {
				if (seconds == 0 && lobby.size() >= 3 || lobby.size() >= AUTO_GAME)
					passPlayersToGame();
				else if (seconds == 0)
					seconds = TIME;
	...
	private void passPlayersToGame() {
		...
		new PestControl(playerList, PestData.valueOf(landerRequirement.name()), false).create();
	}

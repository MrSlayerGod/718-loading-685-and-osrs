...
public final class ItemsContainer<T extends Item> implements Serializable {
	...
	public boolean add(T item) {
		if (alwaysStackable || item.getDefinitions().isStackable() || item.getDefinitions().isNoted()) {
			for (int i = 0; i < data.length; i++) {
				if (data[i] != null) {
					if (data[i].getId() == item.getId() /*&& data[i].getDegrade() == /*item.getDegrade()*/) {
						data[i] = new Item(data[i].getId(), data[i].getAmount() + item.getAmount(), 0/*item.getDegrade()*/);
						return true;
					}
				}
			}
	...
	private boolean hasSpaceForItem(T item) {
		if (alwaysStackable || item.getDefinitions().isStackable() || item.getDefinitions().isNoted()) {
			for (Item aData : data) {
				if (aData != null) {
					if (aData.getId() == item.getId()/* && aData.getDegrade() == item.getDegrade()*/) {
						return true;
					}
				}
			}

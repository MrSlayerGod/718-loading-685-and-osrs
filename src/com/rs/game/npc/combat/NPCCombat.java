...
public final class NPCCombat {
	...
	private int combatAttack() {
		...
		// melee
		int maxDistance = npc.getId() == 27692 ? 3 : attackStyle == NPCCombatDefinitions.MELEE ? 0 : npc.isCantFollowUnderCombat() ? 16 : 10;
		if ((!(npc instanceof Nex)) && !(npc instanceof MuttadileMother) && !npc.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target))) {
			return 0;
		}
		if (npc.hasWalkSteps())
		...
		if (!npc.isCantFollowUnderCombat() && Utils.colides(npc.getX(), npc.getY(), size, target.getX(), target.getY(), targetSize)) {
			return 0;
		}
		addAttackedByDelay(target);
		if(npc.getCustomCombatScript() != null)
			return npc.getCustomCombatScript().attack(npc, target);
	...
	protected void doDefenceEmote(Entity target, Hit hit) {
		...
		int emote = Combat.getDefenceEmote(target);
		if (emote != -1)
			target.setNextAnimationNoPriority(new Animation(emote, hit.getDelay()));
	}
	...
	public void addAttackedByDelay(Entity target) { // prevents multithread
		...
		target.setAttackedBy(npc instanceof Familiar ? ((Familiar)npc).getOwner() : npc);
		...
		if (!npc.isCantFollowUnderCombat()) {
			...
			if (npc instanceof Nex) {
				...
				if (!nex.isFlying() && (!npc.clipedProjectile(target, maxDistance == 0 && !forceCheckClipAsRange(target))) || !Utils.isOnRange(npc.getX(), npc.getY(), size, target.getX(), target.getY(), targetSize, maxDistance)
						&& target instanceof Player && ZarosGodwars.contains((Player) target)) {
					npc.resetWalkSteps();
				...
				// unlike melee
				maxDistance = npc instanceof AlchemicalHydra || npc.getId() == 27692 ? 3 : npc.isForceFollowClose() ? 0 : (attackStyle == NPCCombatDefinitions.MELEE || attackStyle == NPCCombatDefinitions.MAGE_FOLLOW || attackStyle == NPCCombatDefinitions.RANGE_FOLLOW) ? 0 : 7;
				npc.resetWalkSteps();

...
public class SteelTitanCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (usingSpecial) {// priority over regular attack
			...
			int maxHit = target instanceof Player ? 122 : 244; //244;
			if (distant) {// range hit
				delayHit(npc, 2, target, getRangeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.RANGE, target)), getRangeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.RANGE, target)), getRangeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.RANGE, target)), getRangeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.RANGE, target)));
			} else {// melee hit
				delayHit(npc, 1, target, getMeleeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.MELEE, target)), getMeleeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.MELEE, target)), getMeleeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.MELEE, target)), getMeleeHit(npc, getRandomMaxHit(npc, maxHit, NPCCombatDefinitions.MELEE, target)));
			}
		} else {
			if (distant) {

...
public class GluttonousBehemothCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (attackStyle == 0) {
			...
			int damage = getRandomMaxHit(npc, (int) (npc.getMaxHit() * 0.75), NPCCombatDefinitions.MAGE, target);
			...
			delayHit(npc, 2, target, getRangeHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit() * 0.75), NPCCombatDefinitions.RANGE, target)));
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {

	...
	private void sendEntangle(NPC npc, Entity target) {
		...
		if (damage > 0) {
			...
			if (target.getFrozenBlockedDelay() <= Utils.currentTimeMillis()) {
				target.addFreezeDelay(12000, true);
				target.addFrozenBlockedDelay(16000);
			}
				...
				if (t instanceof Player) {
					...
					if (player.getPrayer().hasPrayersOn()) {
						if (!hasDrained) {
							...
							if (prayerPoints > 0) {
								npc.applyHit(new Hit(npc, prayerPoints, HitLook.HEALED_DAMAGE));
								hasDrained = true;
							}
						}
						player.getPrayer().drainPrayer();
						player.setPrayerDelay(15000);// Fifteen seconds

		...
		case 2://Multi magic
			...
			for (Entity t : npc.getPossibleTargets()) {
				...
				if (attack == 0)
					...
					delayHit(npc, 1, t, getMagicHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit() * .95), NPCCombatDefinitions.MAGE, t)));
			}
			break;
		case 3://Bind attacks
		case 4:
			...
			for (Entity t : npc.getPossibleTargets()) {
				...
				if (bindTarget) {
					if (player.getFrozenBlockedDelay() <= Utils.currentTimeMillis()) {
						player.addFreezeDelay(5000, true);
						player.addFrozenBlockedDelay(8000);
					}
					player.setRun(false);
				}
			...
			public void run() {
				...
				if (cycles == 2) {
					...
					targetL: for (Entity target : boss.getPossibleTargets()) {
						...
						player.setRun(false);
						player.addFreezeDelay(5000, true);// Five seconds

		...
		if (Utils.random(5) == 0) {
			...
			for (Entity t : boss.getPossibleTargets()) {
				if (!Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), t.getX(), t.getY(), t.getSize(), 0))
					continue;
				int damage = getRandomMaxHit(npc, (int) npc.getMaxHit(), NPCCombatDefinitions.MELEE, t);
				int damage2 = getRandomMaxHit(npc, (int) npc.getMaxHit(), NPCCombatDefinitions.MELEE, t);
				if (t instanceof Player) {
					Player player = (Player) t;
					if ((damage > 0 || damage2 > 0) && !player.isPrayerBlocked()) {

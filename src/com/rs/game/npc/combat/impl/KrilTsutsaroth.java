...
public class KrilTsutsaroth extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		case 1:// main attack
		case 2:// melee attack
			int MAX_DAMAGE = 463;// normal
			for (Entity e : npc.getPossibleTargets()) {
				if (e instanceof Player && (((Player) e).getPrayer().isMeleeProtecting()) && Utils.random(10) == 0) {
					Player player = (Player) e;
					MAX_DAMAGE = 464;
					npc.setNextForceTalk(new ForceTalk("YARRRRRRR!"));
					npc.playSound(3274, 2);
					player.getPrayer().drainPrayer((Math.round(MAX_DAMAGE / 2)));
					player.setPrayerDelay(Utils.random(5000) + 5000);
					player.getPackets().sendGameMessage("K'ril Tsutsaroth slams through your protection prayer, leaving you feeling drained.");
				}
				npc.setNextAnimation(new Animation(MAX_DAMAGE <= 463 ? 14963 : 14384));
				delayHit(npc, 0, e, getMeleeHit(npc, getRandomMaxHit(npc, MAX_DAMAGE, NPCCombatDefinitions.MELEE, e)));
			}
			break;
		}

...
public class LucienCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (attackStyle == 0) { // normal mage move
			...
			npc.setNextAnimation(new Animation(11338));
			delayHit(npc, 2, target, getRangeHit(npc, getRandomMaxHit(npc, 350, NPCCombatDefinitions.RANGE, target)));
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					for (Entity t : npc.getPossibleTargets()) {
						t.applyHit(new Hit(npc, (int) (t.getHitpoints() * Math.random() * 0.2), HitLook.REGULAR_DAMAGE, 0));
					}
				...
				public void run() {
					...
					for (int dir = 0; dir < dirs[0].length; dir++) {
						...
						if (World.isTileFree(tile.getPlane(), tile.getX(), tile.getY(), size)) { // if
							// found
							// done
							if (tile.getRegionId() == npc.getRegionId())

...
public class NexCombat extends CombatScript {
	...
	/*	boolean hasDistance = false;
		...
		int size = npc.getDefinitions().boundSize;
		if (distanceX > size + 1 || distanceX < -1 || distanceY > size + 1 || distanceY < -1)
			hasDistance = true;*/
		boolean hasDistance = Utils.isOnRange(nex, target, 0);
		nex.resetLastAttack();
		...
		case 1: // Smoke
			...
			case 2:
				...
				break;
			case 3:
				if (Utils.random(3) != 0) //reduce chance of pull
					sendMagicAttack(nex, false);
				else
					sendPullAttack(nex);
				break;
			...
			case 9:
				...
				break;
			case 10:
				if (Utils.random(2) != 0) //reduce chance of pull
					sendMagicAttack(nex, false);
				else
	...
	private void sendIceBarrage(Nex nex) {
		...
		for (final Entity possibleTarget : nex.getPossibleTargets()) {
			...
			int damage = getRandomMaxHit(nex, nex.getMaxHit(), NPCCombatDefinitions.MAGE, possibleTarget);
			...
			if (damage > 0) {
				if (possibleTarget instanceof Player) {
					...
					if (player.getPrayer().usingPrayer(0, 16) || player.getPrayer().usingPrayer(1, 7))
						usingPrayer = true;
				}
	...
	private void sendSipionAttack(final Nex nex, Entity target) {
		...
		int bloodReaverSize = NPCConfig.forID(13458).boundSize;
				...
				if (Utils.random(2) == 0) {
					for (Entity entity : nex.getPossibleTargets()) {
						if (entity instanceof Player) {
							...
							if (distance > 30)
								distance = 30;
							player.getPackets().sendCSVarInteger(1435, (distance * 155 / 30) + 100);
						}
					}
				}
			}
						...
						for (int[] tile : tiles.values()) {
							...
							for (Entity t : possibleTargets)
								if (t.getX() == tile[0] && t.getY() == tile[1])
									t.applyHit(new Hit(nex, Utils.random(400) + 400, HitLook.REGULAR_DAMAGE));
						}
						firstCall = true;
					} else {
	...
	private void sendMeleeAttack(Nex nex, Entity target) {
		...
		Hit hit = getMeleeHit(nex, getRandomMaxHit(nex, nex.getMaxHit(), NPCCombatDefinitions.MELEE, target));
				...
				public void run() {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							player.setPrayerDelay(Utils.random(20000) + 5000);
							player.resetWalkSteps();
	...
	private void sendVirusAttack(Nex nex) {
		...
		for (Entity entity : nex.getPossibleTargets()) {
			...
			entity.applyHit(new Hit(nex, Utils.random(100), HitLook.REGULAR_DAMAGE));
	...
	private void sendMagicAttack(final Nex nex, final boolean secondAttack) {
		for (final Entity possibleTargets : nex.getPossibleTargets()) {
			...
			final int damage = getRandomMaxHit(nex, nex.getMaxHit(), NPCCombatDefinitions.MAGE, possibleTargets);

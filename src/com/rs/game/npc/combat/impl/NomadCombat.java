...
public class NomadCombat extends CombatScript {
	...
	private void spawnFlameVortex(WorldTile tile, boolean hm) {
		if (!World.isFloorFree(tile.getPlane(), tile.getX(), tile.getY()))
			return;
		new FlameVortex(tile, hm);
	}
		...
		if (nomad.isMeleeMode()) {
			...
			if (target instanceof Player && nomad.useSpecialSpecialMove()) {
				...
				case 0:
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							switch (count) {
							case 0:
								spawnFlameVortex(middle.transform(2, 2, 0), nomad.isHardMode());
								break;
							case 1:
								spawnFlameVortex(middle.transform(2, 0, 0), nomad.isHardMode());
								break;
							case 2:
								spawnFlameVortex(middle.transform(2, -2, 0), nomad.isHardMode());
								break;
							...
							case 5:
								...
								break;
							case 6:
								spawnFlameVortex(middle.transform(3, 1, 0), nomad.isHardMode());
								break;
							case 7:
								spawnFlameVortex(middle.transform(3, -1, 0), nomad.isHardMode());
								break;
							case 8:
								spawnFlameVortex(middle.transform(1, -3, 0), nomad.isHardMode());
								break;
							case 9:
								spawnFlameVortex(middle.transform(-1, -3, 0), nomad.isHardMode());
								break;
				...
				case 1:
					...
					WorldTile 	throne = target instanceof Player && 
							((Player)target).getControlerManager().getControler() instanceof TheHorde ?
									((TheHorde) 	((Player)target).getControlerManager().getControler()).getWorldTile(15, 15) : nomad.getThroneTile();
					if (nomad.getX() != throne.getX() || nomad.getY() != throne.getY())
						nomad.sendTeleport(throne);
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							if (!secondLoop) {
								...
								if (npc.clipedProjectile(target, false)) {
									delayHit(npc, 2, target, getRegularHit(npc, nomad.isHardMode() ? 1500 : 750));
				...
				case 3:
					nomad.setCantFollowUnderCombat(true);
					throne = target instanceof Player && 
						((Player)target).getControlerManager().getControler() instanceof TheHorde ?
							((TheHorde) 	((Player)target).getControlerManager().getControler()).getWorldTile(15, 15) :
							nomad.getThroneTile();
					nomad.sendTeleport(throne); //throne

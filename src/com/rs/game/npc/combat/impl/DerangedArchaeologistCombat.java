 ...
 * Oct 31, 2017
 */
public class DerangedArchaeologistCombat extends CombatScript {


	@Override
	public Object[] getKeys() {
		return new Object[] {27806};
	}

	private static final String[] ATTACKS = new String[] {"Learn to Read!","Round and round and round and round!", "The plants! They're alive!", "They came from the ground! They came from the ground!!!", "The doors won't stay closed forever!", "They're cheering! Why are they cheering?", "Time is running out! She will rise again!"};
					...
					if (send2) {
						...
						for (Entity t : npc.getPossibleTargets()) {
							if (t.hasWalkSteps() || Utils.getDistance(newTile.getX(), newTile.getY(), t.getX(), t.getY()) > 1 || !t.clipedProjectile(newTile, false))
								continue;
							delayHit(npc, 0, t, getRegularHit(npc, Utils.random(700)+1));
						}
	...
	public int attack(NPC npc, Entity target) {
		...
		if (attack == 0) {
			...
			npc.setNextAnimation(new Animation(defs.getAttackEmote()));
			delayHit(npc, 1, target, getRangeHit(npc, getRandomMaxHit(npc, attack == 1 ? 700 : npc.getMaxHit(), defs.getAttackStyle(), target)));

...
public class RevenantCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		switch (attackStyle) {
		case 0: // magic
			int damage = noDamage ? 0 : getRandomMaxHit(npc, npc.getMaxHit(), NPCCombatDefinitions.MAGE, target);
			...
			break;
		case 1: // range
			delayHit(npc, 2, target, getRangeHit(npc, noDamage ? 0 : getRandomMaxHit(npc, npc.getMaxHit(), NPCCombatDefinitions.RANGE, target)));
			World.sendProjectile(npc, target, 1278, 34, 16, 30, 35, 16, 0);
			npc.setNextAnimation(new Animation(getRangeAnimation(npc)));
			break;
		case 2: // melee
			delayHit(npc, 0, target, getMeleeHit(npc, noDamage ? 0 : getRandomMaxHit(npc, npc.getMaxHit(), NPCCombatDefinitions.MELEE, target)));

...
public class GodwarsBandosFaction extends NPC {
	...
	private boolean hasGodItem(Player player) {
		for (Item item : player.getEquipment().getItems().getItems()) {
			...
			if(item.getId() == 51733 || name.contains("bandos") || name.contains("book of war") || name.contains("ancient mace") || name.contains("granite mace") || ZarosMinion.isNexArmour(name))
				return true;
		}
		return false;
	}
			...
			public void run() {
				if (loop == 0) {
					...
					if (source instanceof Player) {
						...
						player.getControlerManager().processNPCDeath(GodwarsBandosFaction.this);

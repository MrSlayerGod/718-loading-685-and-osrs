...
public class Nex extends NPC {
	...
	public Nex(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
		...
		setCapDamage(500);
		setLureDelay(3000);
		setDropRateFactor(14);//18 too much
	...
	public void processNPC() {
		...
		if (!getCombat().process())
			checkAgressivity();
		if (!rangeActivated && Utils.random(/*750*/250) == 0 && getId() == 13447) {
	...
	public void handleIngoingHit(Hit hit) {
		...
		if (currentPhase == null)
			return;
		if (getHitpoints() <= getMaxHitpoints() - (currentPhase.getPhaseValue() * 6000) && currentPhase.getPhaseValue() < 5 && !nexMinions[currentPhase.getPhaseValue() - 1].isBarrierBroken()) {

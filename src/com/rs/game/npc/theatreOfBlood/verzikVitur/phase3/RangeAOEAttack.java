...
public class RangeAOEAttack implements TOBAction {
    ...
    public int use(NPC npc) {
        ...
        for (Player target : targetList) {
            //Projectile projectile = new Projectile(6561, npc, target, 6, 0, 30, 30, 41, 5);
            //World.addProjectile(projectile);
        	   int msDelay = World.sendProjectile(npc, target, 6561, 40, 36, 36, 41, 0, npc.getSize() * 32);
        	   ...
        	   WorldTasksManager.schedule(new WorldTask() {
                ...
                public void run() {
                	if (verzik.hasFinished() || !verzik.isRunning())
                		return;
                    target.applyHit(new Hit(verzik, Utils.random(380), HitLook.RANGE_DAMAGE));
                }

...
public class Bork extends NPC {
	...
	public void drop() {
		...
		drops.add(new Item(995, 4000 + Utils.random(50000 * 5))); //coins 
		...
		if (Utils.random(100) == 0) {
			drops.add(new Item(15403, 1)); //balmung
			World.sendNews(controller.getPlayer(), controller.getPlayer().getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(15403).getName() + " <col=ff8c38>drop!", 1);
		}
		for (Item item : drops) {
		...
		//NPCKillLog.addKill(controller.getPlayer(), getName());
	}
	...
	public void spawnMinions() {
		...
		setNextAnimation(new Animation(8757)); //TODO find graphic

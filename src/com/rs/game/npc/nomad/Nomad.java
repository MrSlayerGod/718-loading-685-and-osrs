...
public class Nomad extends NPC {
	...
	public void setNextMovePerformHorde() {
		nextMovePerform = Utils.currentTimeMillis() + Utils.random(40000, 50000);
	}
	...
	public void sendDeath(Entity source) {
		if (throneTile != null && !target.isDead() && !target.hasFinished()) {
			target.lock();
			//target.getVarsManager().sendVarBit(6962, 0); disable tent
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					FadingScreen.fade(target, new Runnable() {
						...
						@Override
						public void run() {
							if (target.hasFinished() || target.isDead() || !(target.getControlerManager().getControler() instanceof NomadsRequiem))
								return;
			...
			},  this.getCombatDefinitions().getDeathDelay() /* getCombatDefinitions().getAttackDelay() + 1*/);
		}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (int i = 0; i < 4; i++) {
					...
					if (thisIndex == i) {
						n = thisNpc;
						setNextWorldTile(getCopySpot(target, i));
					} else {
						n = new FakeNomad(getCopySpot(target, i), thisNpc);
						copies.add(n);
					}
	...
	public WorldTile getCopySpot(Player target, int index) {
		WorldTile 	throneTile = target instanceof Player && 
				((Player)target).getControlerManager().getControler() instanceof TheHorde ?
						((TheHorde) 	((Player)target).getControlerManager().getControler()).getWorldTile(15, 15) : getThroneTile();
	...
	public Item sendDrop(Player player, Drop drop) {
		...
		player.getInventory().addItemDrop(item.getId(), item.getAmount(), NomadsRequiem.OUTSIDE);
		if (drop.getRarity() == Drops.VERY_RARE) 

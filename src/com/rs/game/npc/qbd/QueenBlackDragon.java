...
public final class QueenBlackDragon extends NPC {
	...
	{ 1149, 1, 1, 57 }, // Dragon helm
		...
		{ 24338, 1, 1, 1 }, // royal crossbow
		{ 11286, 1, 1, 1 }, // Draconic visage current: 850
	};
	...
	public void spawnWorm() {
		...
		attacker.getPackets().sendProjectile(null, this, destination, 3141, 128, 0, 60, 0, 5, 3, super.getDefinitions().boundSize);
	...
	public void setPhase(int phase) {
		...
		case 5:
			...
			checkSlayer(attacker);
			//NPCKillLog.addKill(attacker, "Queen Black Dragon");
			break;
		// Qbd is dead roar.
		}
	}
	...
	private void prepareRewards() {
		LuckyPets.checkPet(attacker, LuckyPet.QBD, "Queen Black Dragon");
		...
		for (int i = 0; i < 1 + Utils.random(3); i++) {
			Item item = rewardTable.get(Utils.random(rewardTable.size()));
			rewards.add(item);
			if (item.getId() == 24352 || item.getId() == 24365 || item.getId() == 24338 || item.getId() == 11286) {
				attacker.getCollectionLog().add("Queen Black Dragon", item);
				World.sendNews(attacker, attacker.getDisplayName() + " has received <col=ffff00>" + item.getName() + "<col=ff8c38> drop!", 1);
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		for (int k = 0; k < 10000; k++) {
			...
			ItemsContainer<Item> rewards = new ItemsContainer<>(10, true);
			rewards.add(new Item(536, 5));
			rewards.add(new Item(24372, 1 + Utils.random(6)));
			rewards.add(new Item(24336, 50 + Utils.random(51)));
			List<Item> rewardTable = new ArrayList<Item>();
		...
		System.out.println(bowC+", "+((double)bowC/10000d*100d));

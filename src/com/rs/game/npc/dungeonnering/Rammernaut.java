				...
				public void run() {
					...
					if (ticks == 1) {
						...
						for (int distance = 0; distance < 6; distance++) {
							WorldTile t = new WorldTile(new WorldTile(entity.getX() + (dirs[0] * distance), entity.getY() + (dirs[1] * distance), entity.getPlane()));
							if (World.isWallsFree(t.getPlane(), t.getX(), t.getY()) &&  World.isFloorFree(t.getPlane(), t.getX(), t.getY()) && getManager().isAtBossRoom(t)) 
								tile = t;
							 else 
								break;
						}

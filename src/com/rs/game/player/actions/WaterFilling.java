...
public class WaterFilling extends Action {
	...
	public boolean checkAll(Player player) {
		if (!player.getInventory().containsOneItem(fill.empty)) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You don't have any " + ItemConfig.forID(fill.empty).getName().toLowerCase() + " to fill.");
			return false;
		}
		return true;
	}
	...
	public boolean start(Player player) {
		if (checkAll(player)) {
			...
			String name = ItemConfig.forID(fill.full).getName();
			if (name.contains(" ("))

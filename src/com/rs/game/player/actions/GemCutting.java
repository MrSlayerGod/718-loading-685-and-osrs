...
public class GemCutting extends Action {
	...
	public enum Gem {
		...
		ZENYTE(49496, 49493, 200, 89, 27185),
	...
	public static boolean isGem(int gemId) {
		for (Gem gem : Gem.values()) {
			if (gem.cut == gemId) 
				return true;
		}
	...
	public boolean checkAll(Player player) {
		...
		if (!player.getInventory().containsOneItem(gem.getUncut())) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You don't have any " + ItemConfig.forID(gem.getUncut()).getName().toLowerCase() + " to cut.");
			return false;
		}
		return true;
	}
	...
	public int processWithDelay(Player player) {
		...
		if (gem == Gem.SAPPHIRE)
			player.getAchievements().add(Task.CUT_UNCUT_SAPPHIRE);
		else if (gem == Gem.DIAMOND)
			player.getAchievements().add(Task.CUT_UNCUT_DIAMOND);
		else if (gem == Gem.DRAGONSTONE)
			player.getAchievements().add(Task.CUT_UNCUT_DRAGONSTONE);
		else if (gem == Gem.ONYX)
			player.getAchievements().add(Task.CUT_UNCUT_ONYX);
		
		player.getSkills().addXp(Skills.CRAFTING, gem.getExperience());
		player.getPackets().sendGameMessage("You cut the " + ItemConfig.forID(gem.getUncut()).getName().toLowerCase() + ".", true);
		quantity--;
		if (quantity <= 0)
			return -1;

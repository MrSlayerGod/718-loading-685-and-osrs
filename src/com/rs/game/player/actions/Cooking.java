...
public class Cooking extends Action {
	...
	public static enum Cookables {
		...
		SINEW_2(new Item(2136, 1), 1, 1, 3, new Item(2146, 1), new Item(9436, 1), false, false),
		...
		RAW_RAT_MEAT(new Item(2134, 1), 1, 5, 30, new Item(2146, 1), new Item(2142, 1), false, false),

		RAW_UGTHANKI_MEAT(new Item(1859, 1), 1, 5, 30, new Item(2146, 1), new Item(1861, 1), false, false),

		RAW_RABBIT(new Item(3226, 1), 1, 5, 30, new Item(7222, 1), new Item(3228, 1), false, true),

		RAW_BIRD_MEAT(new Item(9978, 1), 11, 10, 62, new Item(9982, 1), new Item(9980, 1), true, false),

		RAW_CRAB_MEAT(new Item(7518, 1), 21, 20, 100, new Item(7520, 1), new Item(7521, 1), false, false),

		RAW_BEAST_MEAT(new Item(9986, 1), 21, 30, 82, new Item(9990, 1), new Item(9988, 1), true, false),
		...
		POISON_KARAMBWAN(new Item(3142, 1), 1, 20, 80, new Item(3148, 1), new Item(3144, 1), false, false),
		...
		RAW_DARK_CRAB(new Item(41934, 1), 90, 100, 215, new Item(41938, 1), new Item(41936, 1), false, false),

		RAW_ANGLERFISH(new Item(43439, 1), 84, 100, 230, new Item(43443, 1), new Item(43441, 1), false, false),
		...
		public static Cookables forId(short itemId) {
			for (Cookables cook : Cookables.values()) {
				if ((itemId & 0xffff) == cook.getRawItem().getId())
					return cook;
			}
			return null;
		}
	...
	public Cooking(WorldObject object, Item item, int amount, Cookables cook) {
		this.amount = Math.min(28, amount);
		this.item = item;
		this.object = object;
		this.cook = cook;
	}
	...
	public boolean start(Player player) {
		...
		if (cook.isFireOnly() && !object.getDefinitions().getToObjectName(player).contains("Fire")) {
	...
	private boolean isBurned(Cookables cook, Player player) {
		...
		//53 : 50
		return Utils.random(/*34*/(object.getId() == 121302 || player.isAtVipZone() ? 77 : 74)) <= levelsToStopBurn; //reduced burn chance
	}
	...
	public int processWithDelay(Player player) {
		...
		if ((player.getSkills().getLevel(Skills.COOKING) >= cook.getBurningLvl()) ? false : isBurned(cook, player)) {
			...
			player.getPackets().sendGameMessage("Oops! You accidentally burnt the " + cook.getProduct().getDefinitions().getName().toLowerCase() + ".", true);
		} else {
			
			if (cook == Cookables.RAW_SARDINE)
				player.getAchievements().add(Task.COOK_SARDINE);

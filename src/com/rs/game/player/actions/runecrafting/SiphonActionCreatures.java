...
public class SiphonActionCreatures extends Action {
	...
	public boolean checkAll(final Player player) {
		...
		if (creature.hasFinished())
			return false;
		if (!player.withinDistance(creature, 6) || !player.clipedProjectile(creature, true, creature.getDefinitions().boundSize)) {
	...
	public int processWithDelay(final Player player) {
		if (started) {
			...
			if (level <= 50 ? Utils.getRandom(2) == 1 : Utils.getRandom(1) == 1) {
				...
				double totalXp = creatures.xp * Runecrafting.getRCBoost(player);
				...
				LuckyPets.checkPet(player, LuckyPet.RIFT_GUARDIAN_RUNESPAN);
				if (npcLife++ == 10)
	...
	public static boolean chipCreature(Player player, NPC npc) {
		...
		if (!player.getInventory().containsItem(creature.getRuneId(), 10)) {
			player.getPackets().sendGameMessage("You dont have enough " + ItemConfig.forID(creature.getRuneId()).getName() + "s to chip away at that creature.");
			return true;
		} else {

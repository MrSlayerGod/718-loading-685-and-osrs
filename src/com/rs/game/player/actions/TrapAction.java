...
public class TrapAction extends Action {
	...
	public enum HunterNPC {
		...
		BLACK_CHINCHOMPA(Traps.BOX, 73, 315, new int[]
		{ 28906, 5184, -1, 41959 }),

		GRENWALL(Traps.BOX, 77, 1100, new int[]
		{ 28567, 5184, -1, 12539 }),
			...
			public void run() {
				...
				if (captured != null) {
					...
					for (int i = 3; i < ids.length; i++)
						player.getInventory().addItemDrop(ids[i], ids[i] == 12539 ? 20 :
							
							captured == HunterNPC.GREY_CHINCHOMPA || captured == HunterNPC.RED_CHINCHOMPA  || captured == HunterNPC.BLACK_CHINCHOMPA ? 10 :
							1);
					player.getSkills().addXp(Skills.HUNTER, captured.getExp());
					if (captured == HunterNPC.GREY_CHINCHOMPA || captured == HunterNPC.RED_CHINCHOMPA  || captured == HunterNPC.BLACK_CHINCHOMPA)
						LuckyPets.checkPet(player, LuckyPet.BABY_CHINCHOMPA);
				}

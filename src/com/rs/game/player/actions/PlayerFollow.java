...
public class PlayerFollow extends Action {
	...
	private boolean checkAll(Player player) {
		...
		if (player.isFrozen())
			return true;
		Action other = target.getActionManager().getAction() == null ? null : target.getActionManager().getAction();

		if(other != null && other instanceof PlayerFollow && ((PlayerFollow) other).target == player
				&& target.previousWalkTile != null) {
			WorldTile dance = target.previousWalkTile;
			boolean otherTango = ((PlayerFollow) other).tango;
			if(!tango && !otherTango) {
				tango = true;
				return true;
			}
			if(tango && otherTango) {
				((PlayerFollow) other).tango = false;
				player.addWalkSteps(dance.getX(), dance.getY());
				return true;
			}
			if(this.tango && !otherTango) {
				player.addWalkSteps(dance.getX(), dance.getY());
				this.tango = false;
				((PlayerFollow) other).tango = true;
				return true;
			}
		}

...
public class Smithing extends Action {
	...
	public int processWithDelay(Player player) {
		...
		int id = BUILDS[type + (dungeoneering ? 6 : 0)][index];
		
		player.getInventory().deleteItem(new Item(barId, depletedBars));
		
		if (!player.getInventory().addItem(new Item(id, forgedAmount))) {
		...
		if (id == 1117)
			player.getAchievements().add(Task.SMITH_BRONZE_PLATEBODY);
		else if (id == 1329)
			player.getAchievements().add(Task.SMITH_MITHRIL_SCIMITAR);
		else if (id == 1287)
			player.getAchievements().add(Task.SMITH_ADAMANT_SWORD);
		else if (id == 1123)
			player.getAchievements().add(Task.SMITH_ADAMANT_PLATEBODY);
	...
	public static void sendForgingInterface(Player player, int type, boolean dungeoneering) {
		...
		if (!dungeoneering)
			player.getPackets().sendIComponentText(FORGING_INTERFACE, 14, ItemConfig.forID(BARS[0][type]).getName().replace(" bar", "") + " Smithing");

...
public class TrollCutscene extends Cutscene {
	...
	public CutsceneAction[] getActions(final Player player) {
		...
		actionsList.add(new CreateNPCAction(DRAGONKK, 15784, 3092, 3497, 0, "Vision of matrix", 1337, -1));
		actionsList.add(new CreateNPCAction(APACHE, 15783, 3093, 3497, 0, "Soapy", -1, -1));
		actionsList.add(new NPCFaceTileAction(DRAGONKK, 3903, 3497, -1));
		actionsList.add(new NPCFaceTileAction(APACHE, 3902, 3497, -1));

		actionsList.add(new NPCForceTalkAction(DRAGONKK, "It's time for you to die soapy!", 4));
	...
	public void stopCutscene(Player player) {
		//player.getPackets().sendOpenURL("https://www.youtube.com/watch?v=cbJ89efvKqM");
		player.getPackets().sendGameMessage("RIP Apache. Destroyer of Matrix1. Thank cjay0091 and mgi125 for the good memories.");

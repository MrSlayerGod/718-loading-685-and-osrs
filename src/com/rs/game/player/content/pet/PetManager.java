...
public final class PetManager implements Serializable {
	...
	public boolean spawnPet(int itemId, boolean deleteItem) {
		...
		if (npcId > 0) {
			...
			if(pet.getPet() == Pets.TWISTED_OLMLET || pet.getPet() == Pets.ANCESTRAL_OLMLET || pet.getPet() == Pets.ELDER_OLMLET) {
	...
	public void eat(int foodId, Pet npc) {
		...
		if (pets == Pets.TROLL_BABY) {
			...
			//if (trollBabyName == null) {
				trollBabyName = ItemConfig.forID(foodId).getName();
			...
			player.getPackets().sendGameMessage("Your pet happily eats the " + ItemConfig.forID(foodId).getName() + ".");
			return;
		}
		for (int food : pets.getFood()) {
			if (food == foodId) {
				player.getInventory().deleteItem(food, 1);
				player.getPackets().sendGameMessage("Your pet happily eats the " + ItemConfig.forID(food).getName() + ".");
				player.setNextAnimation(new Animation(827));
				npc.getDetails().updateHunger(-15.0);
				return;
			}

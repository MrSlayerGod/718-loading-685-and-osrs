...
public class LastManStandingSurpriseEvent implements SurpriseEvent {
	...
	public void start() {
		...
		GameExecutorManager.fastExecutor.schedule(task = new TimerTask() {
			...
			public void run() {
				try {
					if (phase == 2 || phase == 3)
						LastManStandingSurpriseEvent.this.run();
					else if (phase == 4) {
						task.cancel();
						task = null;
	...
	private void timerinit() {
		...
		World.sendNews(null, "Last man standing event will start in " + PREP_MINS + " minute! Type ::event to get there.", World.WORLD_NEWS);
	}
	...
	public void tryJoin(final Player player) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.getControlerManager().startControler("LastManStandingController", LastManStandingSurpriseEvent.this);
			}
		}, 1);
	}

...
public class Summoning {
	...
	public static void spawnFamiliar(Player player, Pouch pouch) {
		...
		if (player.getSkills().getLevel(Skills.SUMMONING) < pouch.getSummoningCost()) {
			player.getPackets().sendGameMessage("You need "+pouch.getSummoningCost()+" summoning points to use this pouch.");
			return;
		}
		...
		if (pouch == Pouch.UNICORN_STALLION)
			player.getAchievements().add(Task.SUMMON_UNICORN_STALLION);
	...
	public static Familiar createFamiliar(Player player, Pouch pouch) {
		try {
			return (Familiar) Class.forName("com.rs.game.npc.familiar." + (NPCConfig.forID(getNPCId(pouch.getRealPouchId()))).getName().replace(" ", "").replace("ï", "i").replace("-", "").replace("(", "").replace(")", "")).getConstructor(Player.class, Pouch.class, WorldTile.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
			if (!Settings.HOSTED)
				e.printStackTrace();
			return null;
	...
	public static void handlePouchInfusion(Player player, int slotId, int itemId, int creationCount, boolean dungeoneering) {
		...
		ItemConfig def = ItemConfig.forID(pouch.getRealPouchId());
		List<Item> itemReq = def.getCreateItemRequirements(infusingScroll);
		if (itemId == 12268 || itemId == 12089) {
			itemReq.get(1).setAmount(1);
			itemReq.get(2).setId(3226);
			itemReq.get(2).setAmount(1);
			itemReq.get(3).setId(12183);
		}
	...
	public static void sendItemList(Player player, boolean infusingScroll, int count, int slotId, int itemId, boolean dungeoneering) {
		...
		if (pouch == null)
			return;
		if (infusingScroll)
			player.getPackets().sendGameMessage("This scroll requires 1 " + ItemConfig.forID(pouch.getRealPouchId()).name.toLowerCase() + ".");
		else

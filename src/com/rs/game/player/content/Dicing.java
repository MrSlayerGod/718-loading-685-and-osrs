...
public class Dicing {
	...
	public static void privateRoll(final Player player, final int itemId, int graphic, final int lowest, final int highest) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				int roll = getRandom(lowest, highest);//player.getNextRoll() == 0 ? getRandom(lowest, highest) : Math.min(lowest, Math.max(lowest, player.getNextRoll()));
				player.getPackets().sendGameMessage("You rolled <col=db3535>" + roll + "</col> on " + diceText(itemId) + " die.", true);
			}
		}, 1);
	}
	...
	public static void friendsRoll(final Player player, final int itemId, int graphic, final int lowest, final int highest) {
		...
		if (player.withinDistance(Settings.START_PLAYER_LOCATION, 32)) {
			player.getPackets().sendGameMessage("Dicing in this area has been disabled. Do ::dice instead.");
			return;
		}
		player.lock(2);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.getInventory().addItem(itemId, 1);
				int roll = player.getNextRoll() == 0 ? getRandom(lowest, highest) : Math.min(highest, Math.max(lowest, player.getNextRoll()));
				chat.sendLocalMessage(player, "Friends Chat channel-mate <col=db3535>" + player.getDisplayName() + "</col> rolled <col=db3535>" + roll + "</col> on " + diceText(itemId) + " die.");
				player.setNextRoll(0);
			}
		}, 1);
	}

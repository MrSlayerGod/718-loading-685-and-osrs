...
public class House implements Serializable {
	...
	public void enterMyHouse() {
		if (player.getHouse().isBuildMode()) {
			if (!player.getBank().hasVerified(6))
				return;
		}
		joinHouse(player);
	}
	...
	private int getMaxQuantityRooms() {
		...
		int maxRoom = 20 + player.getDonator() * 5;
	...
	public void openBuildInterface(WorldObject object, final Builds build, boolean insideHouse) {
		if (insideHouse && !buildMode) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You can only do that in building mode.");
			return;
		}
		...
		for (int i = 0; i < (interId == 396 ? 7 : 3); i++) {
			if (i >= build.getPieces().length) {
				...
				player.getPackets().sendIComponentText(interId, 14 + (5 * i), ItemConfig.forID(build.getPieces()[i].getItemId()).getName());
				for (int i2 = 0; i2 < 4; i2++)
	...
	public void build(int slot) {
		...
		if (build == null || object == null || build.getPieces().length <= slot)
			return;
		boolean cox = ChambersOfXeric.getRaid(player) != null;
		if(cox && !(build == Builds.CHAMBERS_STORAGE_UNIT || build == Builds.CHAMBERS_STORAGE_UNIT_SMALL || build == Builds.CHAMBERS_STORAGE_UNIT_MED))
			return;
		RoomReference roomref = null;
		if(!cox) {
			int roomX = object.getChunkX() - boundChuncks[0];
			int roomY = object.getChunkY() - boundChuncks[1];
			roomref = getRoom(roomX, roomY, object.getPlane());
			if (roomref == null)
				return;
		}
		final RoomReference room = roomref;
		...
		if(piece.getUpgrade() != null && object.getId() != piece.getUpgrade().getId()) {
			player.sendMessage("You must first build a " + ObjectConfig.forID(piece.getUpgrade().getId()).name + " and upgrade it.");
			return;
		}
		if (player.getRights() < 2) {
			...
			if (build.isWater() ? !hasWaterCan() : ((!player.getInventory().containsOneItem(14112, 14104) && !player.getInventory().containsItemToolBelt(HouseConstants.HAMMER)) || (!player.getInventory().containsItemToolBelt(HouseConstants.SAW) && !player.getInventory().containsOneItem(9625)))) {
		...
		final ObjectReference oref = !cox ? room.addObject(build, slot) : null;
		player.closeInterfaces();
		player.lock();
		player.setNextAnimation(new Animation(build.isWater() ? 2293 : player.getInventory().containsOneItem(14112, 14104) ? 11191 : 3683));
		if (player.getRights() < 2) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if(cox) {
					ChambersOfXeric.getRaid(player).buildStorageUnit(player, piece.getId());
				} else {
	...
	public boolean joinHouse(final Player player) {
		if (!isOwner(player)) { //not owner
			...
			if ((player.isIronman() || ((player.isHCIronman() || player.isUltimateIronman()) && (player.getHcPartner() == null || !player.getHcPartner().equalsIgnoreCase(this.player.getUsername()))))) {
				player.getPackets().sendGameMessage("You can't use this feature as an ironman.");
				return false;
			}
			if ((this.player.isIronman() || this.player.isHCIronman() ||  this.player.isUltimateIronman() && (this.player.getHcPartner() == null || !this.player.getHcPartner().equalsIgnoreCase(player.getUsername())))) {
				player.getPackets().sendGameMessage("You can't enter an ironman house.");
				return false;
			}
		}
		players.add(player);
			...
			for (int y = 0; y < 8; y++) {
				...
				if (objects != null) {
					for (WorldObject object : objects) {
						if (object == null)
							continue;
						ObjectConfig defs = object.getDefinitions();
							...
							if (objects != null) {
								skip: for (WorldObject object : objects) {
									...
									if (object.getDefinitions().containsOption(4, "Build") || object.getDefinitions().name.equals("Habitat space")) {
										if (isDoor(object)) {
											if (!buildMode && object.getPlane() == 2 && getRoom(((object.getX() / 8) - boundChuncks[0]) + Utils.DOOR_ROTATION_DIR_X[object.getRotation()], ((object.getY() / 8) - boundChuncks[1]) + Utils.DOOR_ROTATION_DIR_Y[object.getRotation()], object.getPlane()) == null) {
												WorldObject objectR = new WorldObject(object);
	...
	public static boolean enterHousePortal(Player player) {
		/*if (Settings.HOSTED && !Settings.PVP_WORLD)
		    return false;*/

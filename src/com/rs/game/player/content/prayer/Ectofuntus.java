...
public class Ectofuntus {
	...
	public static boolean handleObjects(final Player player, final int objectId) {
		switch(objectId) {
			...
			case GRINDER_OBJECT:
				if (player.ectofungusBones != -1 && !player.grindedBones) {
			...
			case BIN_OBJECT:
				...
				if (player.ectofungusBones != -1 && player.grindedBones) {
					BoneMeal meal = BoneMeal.forBoneId(player.ectofungusBones);
					if (meal != null) {
			...
			case HOPPER_OBJECT:
				...
				for (Item item : player.getInventory().getItems().getItems()) {
					if (item == null)
						continue;
					BoneMeal meal = BoneMeal.forBoneId(item.getId());
					if (meal != null) 
						return handleItemOnObject(player, item.getId(), HOPPER_OBJECT);
				}
				player.getPackets().sendGameMessage("You do not have any bone to fill the hopper.");
				return true;
		}
		return false;
	}
	...
	public static boolean handleItemOnObject(Player player, int itemId, int objectId) {
		ObjectConfig objectDefs = ObjectConfig.forID(objectId);
		ItemConfig itemDefs = ItemConfig.forID(itemId);
		...
		if (objectId == HOPPER_OBJECT && itemDefs.getName().toLowerCase().contains("bone")) {
			if (player.ectofungusBones != -1) {
				player.getPackets().sendGameMessage("You already have some bones in the hopper.");
				return true;
			}
			...
			if (meal != null) {
				player.ectofungusBones = meal.getBoneId();

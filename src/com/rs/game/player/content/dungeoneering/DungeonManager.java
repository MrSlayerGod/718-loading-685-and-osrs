...
public class DungeonManager {
	...
	public void setKey(KeyDoors key, boolean add) {
		if (add) {
			keyList.add(key);
			for (Player player : party.getTeam())
				player.getPackets().sendGameMessage("<col=D2691E>Your party found a key: " + ItemConfig.forID(key.getKeyId()).getName());
		} else {
			keyList.remove(key);
			for (Player player : party.getTeam())
				player.getPackets().sendGameMessage("<col=D2691E>Your party used a key: " + ItemConfig.forID(key.getKeyId()).getName());
		}
		for (Player player : party.getTeam()) {
	...
	public void sendStartItems(Player player) {
		...
		if (player.isExtremeDonator()) {
			player.getInventory().addItem(17789, 125);
			player.getInventory().addItem(17792, 125);
		}
	}
 ...
 */
	...
	public DungeonNPC spawnNPC(final RoomReference reference, final int id, int x, int y, boolean check, final int type, final double multiplier) {
		...
		final int size = NPCConfig.forID(id).boundSize;
	...
	public WorldObject spawnObject(RoomReference reference, int id, int type, int rotation, int x, int y) {
		...
		ObjectConfig defs = ObjectConfig.forID(id);
	...
	public double[] getBonuses(boolean boss, int level) {
		double[] bonuses = new double[10];

		// less 50% than defence
		bonuses[CombatDefinitions.RANGE_ATTACK] = (int) ((party.getRangeLevel() + (level / 3)) * (boss ? 1 : 0.5));
		bonuses[CombatDefinitions.STAB_ATTACK] = (int) ((party.getAttackLevel() + (level / 3) / 1.2) * (boss ? 1 : 0.5));
		bonuses[CombatDefinitions.MAGIC_ATTACK] = (int) ((party.getAttackLevel() + (level / 3) / 1.5) * (boss ? 1 : 0.5));
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			...
			if (!tickedOff) {
				skip: {if (DungeonConstants.ENABLE_ALL_FLOORS) {

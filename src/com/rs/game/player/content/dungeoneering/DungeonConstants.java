...
public class DungeonConstants {
	...
	public static final boolean ENABLE_ALL_FLOORS = true;
...
;
	...
	public static boolean removeSilkEffect(Entity target, DungeonNPC npc) {
		if (!isVisible(npc, target, true)) {
			...
			npc.setNextAnimation(new Animation(717));
			player.getTemporaryAttributtes().put(Key.SHADOW_SILK_CD, true);//25 secs
			player.getPackets().sendGameMessage("Your concealment effect has been temporarily disabled.");
			WorldTasksManager.schedule(new WorldTask() {

				@Override
				public void run() {
					player.getTemporaryAttributtes().remove(Key.SHADOW_SILK_CD);
	...
	public static boolean isVisible(DungeonNPC npc, Entity target, boolean spell) {
		if (target instanceof Player) {
			...
			if (cape == 15828 || cape == 17279) {
				if (target.getTemporaryAttributtes().get(Key.SHADOW_SILK_CD) != null || (!spell && npc.getShadowType() > NOT_VISIBLE))
					return true;
				return false;
			}
		}

...
public class DungeoneeringFishing extends Action {
	...
	private void addFish(Player player) {
		player.getPackets().sendGameMessage("You manage to catch a " + ItemConfig.forID(spot.getFish().id).getName().toLowerCase() + ".", true);
	...
	private boolean checkAll(Player player) {
		...
		if (!player.getInventory().containsItemToolBelt(FLY_FISHING_ROAD)) {
			player.getPackets().sendGameMessage("You need a " + ItemConfig.forID(FLY_FISHING_ROAD).getName().toLowerCase() + " to fish here.");
			return false;
		}
		if (!player.getInventory().containsOneItem(FEATHER)) {
			player.getPackets().sendGameMessage("You don't have " + ItemConfig.forID(FEATHER).getName().toLowerCase() + " to fish here.");
			return false;
		}

...
public class DungeoneeringFarming {
	...
	public static void initHarvest(final Player player, final Harvest harvest, final WorldObject object) {
		...
		final String productName = ItemConfig.forID(harvest.product).getName().toLowerCase();
	...
	public static void plantHarvest(Item item, final Player player, WorldObject object, DungeonManager dungeon) {
		...
		if (harvest == null)
			...
			player.getPackets().sendGameMessage("You need a Farming level of " + harvest.lvl + " in order to plant a " + ItemConfig.forID(harvest.seed).getName().toLowerCase() + ".");
			return;
		}

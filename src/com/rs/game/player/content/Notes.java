...
public final class Notes implements Serializable {
	...
	private void refresh() {
		for (int i = 0; i < 30; i++)
			player.getPackets().sendCSVarString(149 + i, notes.size() <= i ? "" : notes.get(i).text);
	...
	public boolean add(String text) {
		...
		player.getPackets().sendCSVarString(149 + notes.size(), text);
	...
	public boolean edit(String text) {
		...
		player.getPackets().sendCSVarString(149 + id, text);
		return true;
	}

package com.rs.game.player.content;

import com.rs.game.Animation;
import com.rs.game.Graphics;
import com.rs.game.player.Equipment;
import com.rs.game.player.Player;
import com.rs.game.player.Skills;
import com.rs.game.player.actions.Smithing;

public class DragonfireWard {

	public static void joinPieces(Player player) {
		if (!player.getInventory().containsItemToolBelt(Smithing.HAMMER)) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need a hammer in order to work with the visage.");
			return;
		}
		if (player.getSkills().getLevel(Skills.SMITHING) < 90) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need a Smithing level of 90 to forge a dragonfire ward.");
			return;
		}
		if (!player.getInventory().containsItem(1540, 1)) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need a anti-dragon shield to forge a dragonfire ward.");
			return;
		}
		if (!player.getInventory().containsItem(52006, 1)) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You need a skeletal visage to forge a dragonfire ward.");
			return;
		}
		player.lock(2);
		player.setNextAnimation(new Animation(898));
		player.getInventory().deleteItem(1540, 1);
		player.getInventory().deleteItem(52006, 1);
		player.getInventory().addItem(52003, 1);
		player.getSkills().addXp(Skills.SMITHING, 2000);
		player.getDialogueManager().startDialogue("SimpleDialogue", "Even for an experienced smith it is not an easy task, but eventually it is done.");
	}

	public static void chargeDFW(Player player, boolean fully) {
		int shieldId = player.getEquipment().getShieldId();
		if (shieldId != 52003 && shieldId != 52002)
			return;
		if (shieldId == 52003) {
			player.getEquipment().getItem(Equipment.SLOT_SHIELD).setId(52002);
			player.getEquipment().refresh(Equipment.SLOT_SHIELD);
			player.getAppearence().generateAppearenceData();
		}
		if (player.getCharges().getCharges(52002) == 50) {
			player.getPackets().sendGameMessage("Your dragonfire ward is already full.", true);
			return;
		}
		player.getCharges().addCharges(52002, fully ? 50 : 1, Equipment.SLOT_SHIELD);
		player.getCombatDefinitions().refreshBonuses();
		player.setNextAnimationNoPriority(new Animation(6695));
		player.setNextGraphics(new Graphics(1164));
		player.getPackets().sendGameMessage("Your dragonfire ward glows more brightly.", true);
	}

	public static void empty(Player player) {
		player.lock(1);
		player.getCharges().addCharges(52002, -50, -1);
		player.setNextGraphics(new Graphics(1168));
	//	player.setNextAnimation(new Animation(6700));
		player.getPackets().sendGameMessage("You empty your dragonfire ward.");
	}

	public static boolean isDragonFireShield(int id) {
		if (id == 1540 || id == 52006)
			return true;
		return false;
	}
}

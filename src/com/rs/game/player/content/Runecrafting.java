...
public final class Runecrafting {
	...
	public static void enterSoulAltar(Player player) {
		enterAltar(player, new WorldTile(1815, 3856, 0));
	}
	
	public static void enterWrathAltar(Player player) {
		enterAltar(player, new WorldTile(2335, 4828, 0));
	}
	...
	public static void craftEssence(Player player, int rune, int level, double experience, boolean pureEssOnly, int... multipliers) {
		...
		double totalXp = experience * runes * getRCBoost(player);
		...
		int count = runes * (rune == 9075 ? 1 : 2);
	
		if (rune == 556)
			player.getAchievements().add(Task.CRAFT_AIR_RUNE, count);
		else if (rune == 565)
			player.getAchievements().add(Task.CRAFT_BLOOD_RUNE, count);
		
		player.getInventory().addItem(rune, count); //x5 boost runes from rcing
		player.getPackets().sendGameMessage("You bind the temple's power into " + ItemConfig.forID(rune).getName().toLowerCase() + "s.");
		LuckyPets.checkPet(player, LuckyPet.RIFT_GUARDIAN);
		if (runes >= 20 && Utils.random(200) == 0 && !player.containsItem(RunePouch.ID)) {
			player.getPackets().sendGameMessage("You feel your inventory getting heavier.");
			player.getInventory().addItemDrop(RunePouch.ID, 1);
			World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(RunePouch.ID).getName() + "<col=ff8c38> from <col=cc33ff>runecrafting altar<col=ff8c38>!", 1);
		}
	}

	public static double getRCBoost(Player player) {
		double multiplier = 1;

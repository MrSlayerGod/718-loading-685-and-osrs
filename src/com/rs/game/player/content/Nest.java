...
public class Nest {
	...
	public static void searchNest(final Player player, final int slot) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			@Override
			public void run() {
				int id = getRewardForId(player.getInventory().getItem(slot).getId());
			/*	if (id == 5076 || id == 11964 || id == 5078 || id == 5077)
					World.sendNews(player, player.getDisplayName() + " has received " + ItemConfig.forID(id).getName() + " drop!", 1);

...
public class Enchanting {
	...
	{ 7, 27, 49, 57, 68, 87, 87 };
	private static int[][] JEWLERY_NODES =
	{
	...
	{ 49538, 49535, 49541, 49532 } };
	private static int[][] JEWLERY_PRODUCT =
	{
	...
	{ 719, 720, 721, 719, 720, 720, 720 };
	private static short[] JEWLERY_GRAPHICS =
	{ 114, 115, 115, 153, 154, 154, 154 };
	private static double[] JEWLERY_EXPERIENCE =
	{ 17.5, 37, 59, 67, 78, 97, 110 };
	...
	public static void processMagicEnchantSpell(Player player, int slot, int primaryIndex) {
		...
		if (secondaryIndex == -1) { //zenyte exeption
			for (int i = 0; i < JEWLERY_NODES[6].length; i++) {
				if (item.getId() == JEWLERY_NODES[6][i]) {
					if (primaryIndex != 5) {
						player.getPackets().sendGameMessage("Use enchant onyx jewellery spell on zenyte.");
						return;
					}
					primaryIndex = 6;
					secondaryIndex = i;
		...
		if (secondaryIndex == -1) {
			player.getPackets().sendGameMessage("This spell can not be cast on this item."/*This spell can only be cast on amulets, necklaces, rings, braclets or shapes in the training arena.*/);
			return;

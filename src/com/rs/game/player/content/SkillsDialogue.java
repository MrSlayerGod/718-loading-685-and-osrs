...
public final class SkillsDialogue {
	...
	public static void sendSkillsDialogue(Player player, int option, String explanation, int maxQuantity, int[] items, ItemNameFilter filter, boolean sendQuantitySelector) {
		...
		player.getPackets().sendCSVarInteger(754, option);
		for (int i = 0; i < 10; i++) {
			if (i >= items.length) {
				player.getPackets().sendCSVarInteger(i >= 6 ? (1139 + i - 6) : 755 + i, -1);
				continue;
			}
			player.getPackets().sendCSVarInteger(i >= 6 ? (1139 + i - 6) : 755 + i, items[i]);
			String name = ItemConfig.forID(items[i]).getName();
			if (filter != null)
				name = filter.rename(name);
			player.getPackets().sendCSVarString(i >= 6 ? (280 + i - 6) : 132 + i, (i == 9 ? 0 : (i+1))+". "+name);
		}

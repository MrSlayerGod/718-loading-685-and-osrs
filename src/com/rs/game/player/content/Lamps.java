...
public class Lamps {
	...
	{ 23713, 23714, 23715, 23716, 18782, 51027 };
	public static final int[] SELECTABLE_XP_LAMPS_TYPES = new int[]
	{ LAMP_SMALL, LAMP_MEDIUM, LAMP_BIG, LAMP_HUGE, LAMP_BIG, LAMP_BIG };
	...
	private static void openSelectableDialog(Player player, final int slot, final int id) {
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void start() {
				...
				player.getPackets().sendCSVarInteger(1799, id == 18782 ? 23716 : id);
			...
			public void run(int interfaceId, int componentId) {
				...
				//	}
					...
					double exp = player.getSkills().addXp(selectedSkill, getExp(player.getSkills().getLevelForXp(selectedSkill), selectableLampType(id)) * (player.isHCIronman() || player.isExpert() || player.isUltimateIronman() ? 1 : Settings.XP_RATE), true);
	...
	private static void openSkillDialog(Player player, final int slot, final int id) {
		...
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void run(int interfaceId, int componentId) {
				...
				player.getInventory().deleteItem(slot, new Item(id, 1));
				double exp = player.getSkills().addXp(skillId, getExp(player.getSkills().getLevelForXp(skillId), type)  * (player.isHCIronman() || player.isExpert() || player.isUltimateIronman() || player.isDeadman() ? 1 : Settings.XP_RATE), true);

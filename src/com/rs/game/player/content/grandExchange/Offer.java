...
public class Offer extends Item {
	...
	public boolean collectItems(int slot, int option) {
		...
		if (!defs.isStackable() && (option == (item.getAmount() == 1 ? 0 : 1) || defs.cert == -1)) {
			...
			int properId = defs.cert != -1 ? defs.cert : item.getId();
			if(owner.getInventory().hasFreeSlots(new Item(properId, item.getAmount()))) {
				receivedItems.remove(item);
				item.setId(properId);
				owner.getInventory().addItemMoneyPouch(new Item(defs.cert != -1 ? defs.cert : item.getId(), item.getAmount()));
			} else {
				owner.sendMessage("You cannot hold that many " + item.getName() + ".");
				return false;
			}
		}
		update();
	...
	public void updateStockOffer() {
		if (owner == null)//This check is fine because we only update/reset out stock when the player is online.
			return;
		ExchangeStock stock = owner.getGeManager().getAndCreateStock(getId());
		if (stock == null)
			return;
		int leftFrom = stock.getStock();
		int virtualPrice = (int) (GrandExchange.getPrice(getId()) * 1.5);
		if (virtualPrice > price)
			return;
		int left = getAmount() - totalAmmountSoFar;
		int exchangeAmt = left > leftFrom ? leftFrom : left;
		int totalPrice = exchangeAmt * price;
		int leftcoins = totalPrice - exchangeAmt * virtualPrice;
		if (leftcoins > 0)
			receivedItems.add(new Item(995, leftcoins));
		if (exchangeAmt > 0)
			receivedItems.add(new Item(getId(), exchangeAmt));
		stock.setStock(stock.getStock() - exchangeAmt);
		totalAmmountSoFar += exchangeAmt;
		totalPriceSoFar += totalPrice;
		sendUpdateWarning();
	}

...
public class BarbarianOutpostAgility {
	...
	public static void enterObstaclePipe(final Player player, WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				player.getSkills().addXp(Skills.AGILITY, 1d / 20d * Agility.getAgilityMultiplier(player));
			}
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (!secondLoop) {
					...
					player.setNextWorldTile(toTile);
					player.getSkills().addXp(Skills.AGILITY, 15 * Agility.getAgilityMultiplier(player));
					stop();
				}
			...
			public void run() {
				if (stage == 0) {
					...
					if (getStage(player) == 1) {
						removeStage(player);
						player.getSkills().addXp(Skills.AGILITY, 615 * 2 * Agility.getAgilityMultiplier(player)); //doubled so better than gnome
						if (Utils.random(50) == 0) {
							player.getPackets().sendGameMessage("You feel your inventory getting heavier.");
							if (!player.containsItem(14936)) {
								player.getInventory().addItemDrop(14936, 1);
								World.sendNews(player, player.getDisplayName() + " has received " + ItemConfig.forID(14936).getName() + " from agility!", 1);
							}
						}
					}
					stop();
	...
	public static void swingOnRopeSwing(final Player player, WorldObject object) {
		...
		player.getSkills().addXp(Skills.AGILITY, 22 * Agility.getAgilityMultiplier(player));
	...
	private static void walkAcrossLogBalanceEnd(final Player player, WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.setNextWorldTile(toTile);
				player.getSkills().addXp(Skills.AGILITY, 13 * Agility.getAgilityMultiplier(player));
	...
	public static void walkAcrossBalancingLedge(final Player player, final WorldObject object) {
		if (!Agility.hasLevel(player, 35))
			return;
		player.getPackets().sendGameMessage("You put your feet on the ledge and try to edge across...", true);
		player.lock(5);
	...
	public static void climbObstacleNet(final Player player, WorldObject object) {
		...
		player.getSkills().addXp(Skills.AGILITY, 8.2 * Agility.getAgilityMultiplier(player));
	...
	public static void climbOverCrumblingWall(final Player player, WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.setNextWorldTile(toTile);
				player.getSkills().addXp(Skills.AGILITY, 13.7 * Agility.getAgilityMultiplier(player));
				int stage = getStage(player);
				if (stage == 3)
					setStage(player, 4);
				else if (stage == 4) {
					removeStage(player);
					player.getSkills().addXp(Skills.AGILITY, 46.2 * Agility.getAgilityMultiplier(player));
					player.getAchievements().add(Task.BARBARIAN_AGILITY);
				}
			}

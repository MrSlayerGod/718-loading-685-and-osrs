...
public class ClansManager {
	...
	public void sendMessage(Player player, ChatMessage message) {
		if(!player.checkBankPin()) {
			player.sendMessage("You must enter your bank PIN before attempting that.");
			return;
		}
		...
		Bot.sendLog(Bot.CLAN_CHAT_CHANNEL, "[type=CLAN][clan="+getClan().getClanName()+"][name="+player.getUsername()+"][message="+message.getMessage(false)+"]");
	}
	...
	public static void invite(Player player, Player p2) {
		...
		if (p2.getInterfaceManager().containsScreenInter() || p2.getControlerManager().getControler() != null && p2.getControlerManager().getControler() instanceof StealingCreationLobbyController) {

...
public final class LocalPlayerUpdate {
	...
	private boolean needsRemove(Player p) {
		DungeonPartyManager pt = player.getDungManager().getParty();
		if (pt != null && pt.getDungeon() != null && pt.getTeam().contains(p))
			return false;
		//can't just do this or you'll get chat from other dungeons 
		return p != player && (!p.getCutscenesManager().showYourselfToOthers() || p.hasFinished() || !(player.withinDistance(p, player.hasLargeSceneView() ? 64 : 14) && /*(player.hasLargeSceneView() ||*/ player.getMapRegionsIds().contains(p.getRegionId())));
	}
	...
	private boolean needsAdd(Player p) {
		...
		if (pt != null && pt.getDungeon() != null && pt.getTeam().contains(p))
			return true;
		return p != null && p.getCutscenesManager().showYourselfToOthers() && p.isRunning() && (player.withinDistance(p, player.hasLargeSceneView() ? 64 : 14) && /*(player.hasLargeSceneView() || */player.getMapRegionsIds().contains(p.getRegionId())) && localAddedPlayers < MAX_PLAYER_ADD && p.clientHasLoadedMapRegion();
	}
			...
			if (needsRemove(p)) {
				...
				if (p.hasTeleported()) {
					...
					for (int i2 = i + 1; i2 < localPlayersIndexesCount; i2++) {
						...
						if (needsRemove(p2) || p2.hasTeleported() || p2.getNextWalkDirection() != -1 || (p2.needMasksUpdate() || needAppearenceUpdate(p2.getIndex(),  p2.getAppearence().getMD5AppeareanceDataHash(player))))
							break;
						skip++;
					}
					skipPlayers(stream, skip);
	...
	private void appendUpdateBlock(Player p, OutputStream data, boolean needAppearenceUpdate, boolean added) {
		...
		if (p.getNextGraphics2() != null)
			maskData |= 0x8000;
		if (!p.getNextHits().isEmpty() || p.isRefreshHitbars())
			maskData |= 0x8;
	...
	private void applyHitsMask(Player p, OutputStream data) {
		...
		data.writeBigSmart(p.getHitbarSprite(player));

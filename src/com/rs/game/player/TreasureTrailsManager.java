...
public class TreasureTrailsManager implements Serializable {
	...
	private int medCluesCopleted = 0, hardCluesCompleted = 0, eliteCluesCompleted = 0;
	...
	private int generateClueSize(int level) {
		switch (level) {
		case EASY:
			return 2;//1 + Utils.random(4);
		case MEDIUM:
			return 3;//2 + Utils.random(5);
		case HARD:
			return 4;//3 + Utils.random(7);
		case ELITE:
			return 5;////5 + Utils.random(5);
		}
		return 1;
	}
	...
	public void setNextClue(int source) {
		...
		if (cluePhase == lastPhase) {
			...
			if (source == SOURCE_DIG) {
				...
				currentClue.details = generateClueDetails(currentClue.dificulty);
				player.getDialogueManager().startDialogue("SimpleItemMessage", CLUE_SCROLLS[currentClue.dificulty], NPCConfig.forID(id).getName() + " has given you another clue scroll.");
		...
		if (requiredItems.length > 0) {
			for (Item item : requiredItems) {
				...
				if (requestedItem == null)
					return;
				if (item.getId() == 3333 && requestedItem.getId() == 3343)
					continue;
	...
	private Item[] generateRewards(int level) {
		Item[] rewards;
		if (level == EASY) {
			rewards = new Item[3 +  (Utils.random(2) == 0 ? 1 : 0)];
			...
			if (rewards.length == 4) {
				...
				World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + rewards[3].getName() + "<col=ff8c38> from <col=cc33ff>a clue scroll<col=ff8c38>!", 1);
			}
		} else if (level == MEDIUM) {
			rewards = new Item[5 +  (Utils.random(2) == 0 ? 1 : 0)];
			rewards[0] = OVERALL_REWARDS[Utils.random(OVERALL_REWARDS.length)];
			...
			if (rewards.length == 6) {
				...
				World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + rewards[5].getName() + "<col=ff8c38> from <col=cc33ff>a clue scroll<col=ff8c38>!", 1);
			}
		} else if (level == HARD) {
			rewards = new Item[7 +  (Utils.random(3) == 0 ? 1 : 0)];
			rewards[0] = OVERALL_REWARDS[Utils.random(OVERALL_REWARDS.length)];
			...
			if(Utils.random(20) == 0) {
				player.getInventory().addItemDrop(Mimic.MIMIC_CASKET, 1);
				player.sendMessage("<col=ff0000>You find a Mimic crate!");
			}
			if (rewards.length == 8) {
				rewards[7] = HARD_RARE_REWARDS[Utils.random(HARD_RARE_REWARDS.length)];
				World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + rewards[7].getName() + "<col=ff8c38> from <col=cc33ff>a clue scroll<col=ff8c38>!", 1);
			}
		} else { //elite
			rewards = new Item[8 +  (Utils.random(6) == 0 ? 2 : (Utils.random(6) == 0 ? 1 : 0))];
			rewards[0] = OVERALL_REWARDS[Utils.random(OVERALL_REWARDS.length)];
			...
			if (rewards.length >= 9) {
				...
				World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + rewards[8].getName() + "<col=ff8c38> from <col=cc33ff>a clue scroll<col=ff8c38>!", 1);
			}
			if (rewards.length == 10) {
				rewards[9] = ELITE_RARE_REWARDS[Utils.random(ELITE_RARE_REWARDS.length)];
				World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + rewards[9].getName() + "<col=ff8c38> from <col=cc33ff>a clue scroll<col=ff8c38>!", 1);
			}
		}
		int count = 0;
		...
		rewards[rewards.length-1] = new Item(995, (HARD+2) * 100000);
		return rewards;
	}
	...
	public void openReward(int level) {
		...
		player.setCloseInterfacesEvent(new Runnable() {
			...
			public void run() {
				for (Item item : rewards) {
					...
					String rewardLevel = level==ELITE?"Elite":level==HARD?"Hard":level==MEDIUM?"Medium":"Easy";
					player.getCollectionLog().add(CategoryType.CLUES, rewardLevel + " Clues", item);
				/*	if (item.getId() == 42437 || item.getId() == 42426 || item.getId() == 42424 || item.getId() == 42422 || item.getId() == 50011 || item.getId() == 50014 || 
		...
		LuckyPets.checkPet(player, LuckyPet.BLOOHOUND);
		cluesCompleted++;
		if(level == ELITE) {
			incrementEliteClues();
		} else if(level == HARD) {
	...
	public boolean hasSextantItems() {
		return player.getInventory().containsItemToolBelt(2575) && player.getInventory().containsItemToolBelt(2576) && (true || player.getInventory().containsItem(2574, 1));
	}
	...
	public void incrementCluesCompleted() {
		cluesCompleted++;
	}

	public int getMedCluesCompleted() {
		return medCluesCopleted;
	}

	public void incrementMedClues() {
		medCluesCopleted++;
	}

	public int getHardCluesCompleted() {
		return hardCluesCompleted;
	}

	public void incrementHardClues() {
		hardCluesCompleted++;
	}

	public int getEliteCluesCompleted() {
		return eliteCluesCompleted;
	}

	public void incrementEliteClues() {
		eliteCluesCompleted++;
	}
	...
	private static enum ClueDetails {
		...
		SIMPLE_S_1(EASY, SIMPLE, OBJECT, 66875, "Search a barrel near the Combat", "Instructor at the combat", "training area, in Burthorpe."),
		...
		SIMPLE_S_27(EASY, SIMPLE, OBJECT, 111600, "Search the crates in", "Gunnarsgrunn (Barbarian Village)", "helmet shop."),
		...
		{ 15, 4 }, new Item[]
		{ new Item(3333, 1), new Item(1381, 1) }, "Dance a jig", "under Shantay's Awning.", "Bow or curtsy before you talk to me.", "Equip a bruise blue", "snail helmet, air staff."),
	...
	{ new Item(995, 20000 * 10) //coins coins boosted by 10x
		,
		new Item(3827),
		new Item(3828),
		new Item(3829),
	...
	public static final Item[] EASY_RARE_REWARDS =
	...
	public static final Item[] MEDIUM_RARE_REWARDS =
	...
	public static final Item[] HARD_RARE_REWARDS =
	{ new Item(2623), new Item(2625), new Item(2627), new Item(2629), new Item(3477) //Trimmed rune armour
		...
		new Item(42424),
		new Item(42426),
		new Item(42437) //Third-age osrs armour
		, 

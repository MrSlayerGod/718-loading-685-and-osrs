...
public final class LocalNPCUpdate {
	...
	private void appendUpdateBlock(NPC n, OutputStream data, boolean added) {
		...
		if (!n.getNextHits().isEmpty() || n.isRefreshHitbars()) {
			maskData |= 0x4;
		}
	...
	private void applyHitMask(NPC n, OutputStream data) {
		...
		data.writeBigSmart(n.getHitbarSprite(player));
		...
		if (count > 0) {
			...
			if (hp > maxHp)
				hp = maxHp;
			int hpBarPercentage = hp * 255 / Math.max(1, maxHp);
	...
	private void applyAnimationMask(NPC n, OutputStream data) {
		for (int id : n.getNextAnimation().getIds())
			data.writeBigSmart(player.isOldNPCLooks() ? CustomNPCs.getAnim(n.getId(), id) : id);
	...
	private void applyGraphicsMask4(NPC n, OutputStream data) {
		data.writeShort128(
				...
				player.isOldNPCLooks() ? CustomNPCs.getGFX(n.getId(), n.getNextGraphics4().getId()) : n.getNextGraphics4().getId()
				);
	...
	private void applyGraphicsMask3(NPC n, OutputStream data) {
		data.writeShortLE128(player.isOldNPCLooks() ? CustomNPCs.getGFX(n.getId(), n.getNextGraphics3().getId()) : n.getNextGraphics3().getId());
	...
	private void applyGraphicsMask2(NPC n, OutputStream data) {
		data.writeShort128(player.isOldNPCLooks() ? CustomNPCs.getGFX(n.getId(), n.getNextGraphics2().getId()) : n.getNextGraphics2().getId());
	...
	private void applyGraphicsMask1(NPC n, OutputStream data) {
		data.writeShortLE128(player.isOldNPCLooks() ? CustomNPCs.getGFX(n.getId(), n.getNextGraphics1().getId()) : n.getNextGraphics1().getId());

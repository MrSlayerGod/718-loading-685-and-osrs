	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			if (player.getSlayerManager().getCurrentMaster() != master || player.getSlayerManager().getCurrentTask() == null) {
				downgradeTask = master == SlayerMaster.TURAEL && player.getSlayerManager().getCurrentMaster() != SlayerMaster.TURAEL;

				if(!downgradeTask) {
					sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Please give me a task.", "What do you have in your shop?", "Nothing, Nevermind.");
				} else {
					sendOptionsDialogue(DEFAULT_OPTIONS_TITLE, "Please give me a task.", "What do you have in your shop?", "I'd like an easier task.", "Nothing, Nevermind.");
				}
				stage = 1;
			} else {
			...
			if (componentId == OPTION_1) {
				...
				sendNPCDialogue(npcId, 9827, "Only the best slayer equipment money could buy. Come check it out.");
				stage = 5;
			} else if(downgradeTask && componentId == OPTION_3) {
				end();

...
public class SpinningD extends Dialogue {
	...
	public void run(int interfaceId, final int componentId) {
		...
		if (componentIndex > INGREDIENT[dung ? 1 : 0].length) {
			end();
			return;
		}
		player.getActionManager().setAction(new Action() {
			...
			public boolean start(final Player player) {
				...
				if (player.getSkills().getLevel(Skills.CRAFTING) < levelReq) {
					...
					player.getPackets().sendGameMessage("You need a Crafting level of " + levelReq + " in order to spin a " + ItemConfig.forID(PRODUCT[dung ? 1 : 0][componentIndex]).getName().toLowerCase() + ".");
					return false;
				}
				int leatherAmount = player.getInventory().getAmountOf(INGREDIENT[dung ? 1 : 0][componentIndex]);
				if (leatherAmount == 0) {
					player.getPackets().sendGameMessage("You need a piece of " + ItemConfig.forID(INGREDIENT[dung ? 1 : 0][componentIndex]).getName().toLowerCase() + " in order to make a " + ItemConfig.forID(PRODUCT[dung ? 1 : 0][componentIndex]).getName().toLowerCase() + ".");
					end();
					return false;
				}

...
public class KaramjaTrip extends Dialogue {
	...
	public void start() {
		...
		{ NPCConfig.forID(npcId).name, "Do you want to go on a trip to Karamja?" }, IS_NPC, npcId, 9827);
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]
			{ NPCConfig.forID(npcId).name, "The trip will cost you 30 coins." }, IS_NPC, npcId, 9827);
			stage = 0;
		} else if (stage == 0) {

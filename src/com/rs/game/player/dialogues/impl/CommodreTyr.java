...
public class CommodreTyr extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			stage = 0;
			sendNPCDialogue(npcId, 9827, player.getDisplayName()+", it's good to see you. What can I do for you?");
		} else if (stage == 0) {
			stage = 1;
			...
			stage = 3;
			sendNPCDialogue(npcId, 9827, "Ah yes, it's returned home. I can give you it to you again, but I'll have to ask you for 5,000,000 coins to honour those who died in the Pest Queen conflict.");
		} else if (stage == 3) {
			stage = 4;
			sendOptionsDialogue("Will you pay 5,000,000 coins to retreive the sword?", "Yes.", "No.");
		} else if (stage == 4) {
			stage = 5;
			if (componentId == OPTION_1) {
				if (player.getInventory().getCoinsAmount() >= 5000000) { //250k original

...
public class RuneScapeGuide extends Dialogue {
	...
	public void start() {
		...
		if (s == 0) {
			sendEntityDialogue(SEND_2_TEXT_CHAT,
					new String[] { NPCConfig.forID(npcId).name,
							"Greetings! I see you are a new arrival in this land. My",
							"job is welcome all new visitors. So Welcome!" },
					IS_NPC, npcId, 9827);
		} else if (s == 2) {
			sendEntityDialogue(SEND_1_TEXT_CHAT,
					new String[] { NPCConfig.forID(npcId).name, "I'm glad you're making progress!" },
					IS_NPC, npcId, 9827);
			stage = 5;
		} else
			end();
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			stage = 3;
			sendEntityDialogue(SEND_2_TEXT_CHAT, new String[] { NPCConfig.forID(npcId).name,

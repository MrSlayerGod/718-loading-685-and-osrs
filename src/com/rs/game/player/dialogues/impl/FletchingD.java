...
public class FletchingD extends Dialogue {
	...
	public void start() {
		...
		if(items.getProduct().length == 1 && items.name().toLowerCase().contains("dart")) {
			// create darts quickly like osrs
			player.getActionManager().setAction(new Fletching(items, 0, 1));
			return;
		}

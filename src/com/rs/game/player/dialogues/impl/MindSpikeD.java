...
public class MindSpikeD extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		int itemId = player.getInventory().getItem(slot) == null ? -1 : player.getInventory().getItem(slot).getId();
		if(itemId == 23044 || itemId == 23045 || itemId == 23046 || itemId == 23047) {
			if (componentId != OPTION_4) {
				player.getInventory().replaceItem(
						getTransformationForId(componentId == OPTION_1 ? 0 : componentId == OPTION_2 ? 1 : 2), 1, slot);

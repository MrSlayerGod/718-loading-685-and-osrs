...
public class FarmingManager implements Serializable {
	...
	public static enum ProductInfo {
		...
		Calquat(5503, 72, 5980, 0, 129.5, 1016/*2016*/, 160, 8, CALQUAT)
		;
	...
	public static enum SpotInfo {
		...
		Farming_guild_Tree(133732, TREES),
		...
		Farming_guild_Fruit_Tree(134007, FRUIT_TREES),
		...
		Hosidius_Allotment_North(127113, ALLOTMENT),
		Hosidius_Allotment_South(127114, ALLOTMENT),
		Farming_Allotment_Guild_North(133694, ALLOTMENT),
		Farming_Allotment_Guild_South(133693, ALLOTMENT),
		Prifddinas_Allotment_North(134922, ALLOTMENT),
		Prifddinas_Allotment_South(134921, ALLOTMENT),
		...
		Hosidius_Flower(127111, FLOWERS),
		Farming_Guild_Flower(133649, FLOWERS),
		Prifddinas_Flower(134919, FLOWERS),

		Champions_Bush(7577, BUSHES),
		Rimmington_Bush(7578, BUSHES),
		Etceteria_Bush(7579, BUSHES),
		South_Arddougne_Bush(7580, BUSHES),
		Farming_Guild_Bush(134006, BUSHES),

		Falador_Herb_Patch(8150, HERBS),
		Catherby_Herb_Patch(8151, HERBS),
		Ardougne_Herb_Patch(8152, HERBS),
		Canfis_Herb_Patch(8153, HERBS),
		Trollhiem_Herb_patch(18816, HERBS),
		Hosidius_Herb_patch(127115, HERBS),
		Farming_Guild_Herb_patch(133979, HERBS),
		...
		Farming_Guid_Cactus(133761, CACTUS),
		
		Calquat(7807, CALQUAT)
		;

		private static Map<Integer, SpotInfo> informations = new HashMap<Integer, SpotInfo>();
		...
		private SpotInfo(int objectId, int type) {
			...
			this.configFileId = ObjectConfig.forID(objectId).configFileId;
			this.type = type;
		}
	}
	...
	public void startHarvestingAction(final FarmingSpot spot) {
		...
		player.getActionManager().setAction(new Action() {
			...
			public int processWithDelay(Player player) {
				...
				LuckyPets.checkPet(player, LuckyPet.TANGLEROOT);
				return 2;
			}
	...
	private void startPickingAction(final FarmingSpot spot) {
		player.getActionManager().setAction(new Action() {
			...
			@Override
			public int processWithDelay(Player player) {
				player.getPackets().sendGameMessage("You pick a " + ItemConfig.forID(spot.productInfo.productId).getName().toLowerCase() + ".");
				player.setNextAnimation(getHarvestAnimation(spot.productInfo.type));
				player.getSkills().addXp(Skills.FARMING, spot.productInfo.experience);
				player.getInventory().addItemDrop(spot.productInfo.productId, 1);
				spot.harvestAmount--;
	...
	private void collectTreeProducts(final FarmingSpot spot, final TreeDefinitions definitions) {
		player.getActionManager().setAction(new Action() {
			...
			@Override
			public int processWithDelay(Player player) {
				Woodcutting.addLog(hatchet, definitions, false, false, player);
	...
	public class FarmingSpot implements Serializable {
		...
		public void setIdle() {
			...
			refresh();
			resetCycle();
			cycleTime += 30000L * 3; //delay herbs from growing right awaay
		}
		...
		public void refresh() {
			...
			if (spotInfo.objectId >= Settings.OSRS_OBJECTS_OFFSET)
				player.getVarsManager().sendVar(spotInfo.objectId, value);
			else
				player.getVarsManager().sendVarBit(spotInfo.configFileId, value);
			if (Settings.DEBUG)
				System.out.println("Refresh Value: " + value+", "+spotInfo.configFileId+", "+spotInfo.objectId);
		}

...
public class DungManager implements Serializable {
	...
	public void setPlayer(Player player) {
		...
		if (DungeonConstants.ENABLE_ALL_FLOORS) {
			maxFloor = 60;
			maxComplexity = 6;
		}
	}
	...
	public void bind(Item item, int slot) {
		...
		if (DungeonUtils.isBindAmmo(item)) {
			...
			int extraSlots = player.isSupremeVIPDonator() ? 4 : player.isVIPDonator() ? 3 : player.isLegendaryDonator() ? 2 : player.isExtremeDonator() ? 1 : 0;
			if (bindedItems.getUsedSlots() >= 
					extraSlots + DungeonUtils.getMaxBindItems(player.getSkills().getLevelForXp(Skills.DUNGEONEERING))) {
				player.getPackets().sendGameMessage("A currently bound item must be destroyed before another item may be bound.");
				return;
			}
			item.setId(bindId);
	...
	public void reset() {
		...
		maxFloor = DungeonConstants.ENABLE_ALL_FLOORS ? 60 : 1;
		maxComplexity = DungeonConstants.ENABLE_ALL_FLOORS ? 6 : 1; //originally 1.
		visitedResources = new boolean[ResourceDungeon.values().length];
		cVersion = version;
	}
	...
	public void openPartyInterface() {
		player.getInterfaceManager().sendQuestTab(939);
	...
	public void pressOption(int playerIndex, int option) {
		...
		if (option == 0) {
			...
			if (player.isCantWalk() || player.isLocked() || player.isCantWalk() || dungeon != null && dungeon.isBossOpen()) {
	...
	public void enterDailyDungeon(int challengeId) {
		player.stopAll();
		expireInvitation();
		
		DailyDungeon.checkForUpdates();
		if (challengeId < 0 || challengeId >= 5) {
			player.getPackets().sendGameMessage("Wrong challengeId!");
			return;
		}
		
		long seed = DailyDungeon.SEEDS[challengeId];
		int floor = DailyDungeon.FLOORS[challengeId];
		...
		for (Player p2 : party.getTeam()) {
			if (DailyDungeon.hasCompleted(p2, challengeId)) {
				player.getPackets().sendGameMessage(p2.getDisplayName() + " has already tried this challenge.");
				return;
			}
		}
		...
		if (party.getDificulty() != (challengeId+1)) {
			player.getPackets().sendGameMessage("Your team size must be " + (challengeId+1) + " for this challenge.");
			return;
		}
		
		
		if (party.getMaxFloor() < floor) {
			player.getPackets().sendGameMessage("A member in your party can't do floor " + floor + ".");
			return;
		}
		party.setFloor(floor);
		
		
		if (party.getMaxComplexity() < 6) {
			player.getPackets().sendGameMessage("A member in your party can't do complexity 6.");
			return;
		}
		party.setComplexity(6);
		party.setSize(DungeonConstants.LARGE_DUNGEON);
		party.setKeyShare(false);
		party.setGuideMode(false);
		party.setCustomSeed(seed);
		refreshPartyGuideModeComponent();
		
		for (Player p2 : party.getTeam()) {
			/*if (p2.isIronman() && party.getTeam().size() > 1) {
		...
		for (Player p2 : party.getTeam())
			DailyDungeon.setCompleted(p2, challengeId);
		party.start(1);
	}
	...
	public void refreshNames() {
		...
		int index = 0;
		for (Player p2 : party.getTeam())
			player.getPackets().sendCSVarString(292 + (index++), p2.getDisplayName());
	}

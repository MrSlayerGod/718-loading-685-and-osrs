...
public final class DominionTower implements Serializable {
	...
	public static final Item[] REWARD_CHEST = new Item[]
	{
		//nightmare rewards
		new Item(232, 10),
		new Item(224, 10),
	...
	private void teleportToArena(int mode) {
		...
		player.getMusicsManager().playMusic(MUSICS[Utils.random(MUSICS.length)]);  //fixed
	}
	...
	public void startFight(final NPC[] bosses) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			private int count = dominionFactor > 10000 ? 8 : 0;
	...
	public void win(int mode) {
		...
		totalScore += factor;
		DTRank.checkRank(player, mode, getMap().getStringValue(2095));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			private int count = dominionFactor > 10000 ? 4 : 0;
	...
	public NPC[] createBosses() {
		...
		for (int i = 0; i < bosses.length; i++) {
			...
			String name = NPCConfig.forID(id).name;
			for (int i2 = 0; i < id; i2++) {
				NPCConfig def = NPCConfig.forID(i2);
	...
	public void selectBoss() {
		if (nextBossIndex < 0) {
			nextBossIndex = Utils.random(48)+1;
			...
			if (nextBossIndex < 0)
				nextBossIndex = 1;
			player.getPackets().sendExecuteScript(5435, 0, nextBossIndex);
		} else
			player.getPackets().sendExecuteScript(5435, 1, nextBossIndex);
	}
	...
	public int getBossesTotalLevel() {
		...
		for (int i = 0; i < count; i++)
			level += NPCConfig.forID(map.getIntValue(2168 + i)).combatLevel;
		return level;
	}
	...
	public void openRewardsChest() {
		...
		for (int i = 0; i < rewards.getSize(); i++) {
			...
			if (item.getId() >= 22358 && item.getId() <= 22369) {
				player.getCollectionLog().add(CategoryType.MINIGAMES, "Dominion Tower", item);
				announceGloves = true;
			}
		}
		if (announceGloves)
			World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>some gloves<col=ff8c38> from <col=cc33ff>the dominion tower<col=ff8c38>!", 1);
		progress = 0;
		dominionFactor = 0;

...
public class DeathEvent extends Controller {
	...
	public static WorldTile getRespawnHub(Player player) {
		return new WorldTile(3094, 3502, 0);//Settings.START_PLAYER_LOCATION;//HUBS[getCurrentHub(player)];
	}
				...
				try {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							player.getMusicsManager().playMusic(721);  //fixed
	...
	public void getReadyToRespawn() {
		if (!player.getBank().hasVerified(10))
			return;
		slots = GraveStone.getItemSlotsKeptOnDeath(player, false, hadSkull(), protectItem);
		...
		player.getPackets().sendHideIComponent(18, 27, true); //choose respaawn
		player.setCloseInterfacesEvent(new Runnable() {

			@Override
			public void run() {
				WorldTile respawnTile = getRespawnHub(player);//currentHub >= 256 ? RESPAWN_LOCATIONS[currentHub - 256] : HUBS[currentHub];
				synchronized (slots) {

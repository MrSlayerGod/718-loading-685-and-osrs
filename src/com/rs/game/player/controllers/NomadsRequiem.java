...
public class NomadsRequiem extends Controller {
	...
	public static void enterNomadsRequiem(Player player, boolean hardMode) {
		player.getControlerManager().startControler("NomadsRequiem", hardMode);
	}

	@Override
	public void start() {
		hardMode = (boolean) this.getArguments()[0];
		enter(player.getQuestManager().completedQuest(Quests.NOMADS_REQUIEM) ? DungeonPart.THRONE : DungeonPart.ENTRANCE, 0);
	...
	public void enterDoor(int doorIndex) {
		...
		player.getMusicsManager().playMusic(currentPart == DungeonPart.THRONE ? 739 : 734); //fixed
		if (currentPart == DungeonPart.THRONE) {
			...
			if (player.getQuestManager().completedQuest(Quests.NOMADS_REQUIEM)) {
				...
				player.setNextWorldTile(new WorldTile(getBaseX() + 16, getBaseY() + 19, 0));
				continueThroneScene();
			} else
				startThroneScene();

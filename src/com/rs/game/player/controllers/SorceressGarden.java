...
public class SorceressGarden extends Controller {
	...
	public enum Gate {
		...
		WINTER(21709, 1, new WorldTile(2902, 5470, 0), new WorldTile(2903, 5470, 0), 464),
		SPRING(21753, 25, new WorldTile(2921, 5473, 0), new WorldTile(2920, 5473, 0), 463),
		AUTUMN(21731, 45, new WorldTile(2913, 5462, 0), new WorldTile(2913, 5463, 0), 465),
		SUMMER(21687, 65, new WorldTile(2910, 5481, 0), new WorldTile(2910, 5480, 0), 462);
		...
		public static void handleGates(Player player, int objectId, int lvlReq, WorldTile toTile, int musicId) {
			...
			player.getMusicsManager().playMusic(musicId); //fixed
		}
	...
	public boolean processObjectClick1(WorldObject object) {
		if (object.getId() == 21764) {
			...
			if (Utils.random(10) == 0)
				dropSet(player);
			teleMiddle();
			player.incrementSorceressGardenCompletions();
	...
	public static final int[] PIECES = {21480, 21481, 21482, 21483};
	...
	private void dropSet(Player player) {
		...
		player.getCollectionLog().add(CategoryType.MINIGAMES, "Sorceress Garden", new Item(piece));
		player.getPackets().sendGameMessage("You feel your inventory getting heavier.");
		player.getInventory().addItemDrop(piece, 1);
		World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(piece).getName() + "<col=ff8c38> from <col=cc33ff>sorceress's garden<col=ff8c38>!", 1);

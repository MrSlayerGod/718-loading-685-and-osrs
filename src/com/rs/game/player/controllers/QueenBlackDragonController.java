...
public final class QueenBlackDragonController extends Controller {
	...
	public static final WorldTile OUTSIDE = new WorldTile(1197, 6499, 0);
				...
				try {
					...
					FadingScreen.unfade(player, time, new Runnable() {
						...
						public void run() {
							...
							player.getMusicsManager().playMusic(1082); // AWOKEN fixed
						}
					});
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		if (object.getId() == npc.getActiveArtifact().getId()) {
			player.getMusicsManager().playMusic(1083); // QUEEN BLACK DRAGON fixed
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		...
		case 1284:
			switch (componentId) {
			case 8:
				int space = player.getBank().addItems(npc.getRewards().toArray(), false);
				if (space == 0) {

...
public class FightKiln extends Controller {
	...
	{ 1044, 1050, 1051 };
	public static final int TOKHAAR_KAL = 23659;

	public void playMusic() {
		player.getMusicsManager().playMusic(selectedMusic); //fixed
	}
	...
	public void spawn(int index) {
		...
		for (int i = index; i < (index + 4 > WAVES[currentWave - 1].length ? WAVES[currentWave - 1].length : index + 4); i++) {
			...
			if (npcId == 15213)
				new TokHaarKetDill(WAVES[currentWave - 1][i], getSpawnTile(i, NPCConfig.forID(npcId).boundSize), this);
			else
				new FightKilnNPC(WAVES[currentWave - 1][i], getSpawnTile(i, NPCConfig.forID(npcId).boundSize), this);
		...
		if (type == 0) {
			...
			if (type == 1 || type == 4) {
				...
				if (type == 4) {
					...
					World.sendNews(player, player.getDisplayName() + " has completed <col=D80000>Fight Kiln<col=ff8c38>!", 1);
					player.getPackets().sendGameMessage("You were victorious!!");
					LuckyPets.checkPet(player, LuckyPet.SHRIMPY, "Fight Kiln");
					Integer reward = (Integer) player.getTemporaryAttributtes().get("FightKilnReward");
					int itemId = reward != null && reward == 1 ? 6571 : TOKHAAR_KAL;
					player.incrementFightKilnCompletions();
					player.getCollectionLog().add(CategoryType.MINIGAMES, "Fight Kiln", new Item(reward));
	...
	public int getCurrentWave() {
		if (getArguments() == null || getArguments().length == 0)
			return 0;
		return getArguments()[0] == null ? 0 : (Integer) getArguments()[0];
	}

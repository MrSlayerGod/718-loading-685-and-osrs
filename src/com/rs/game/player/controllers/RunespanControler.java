...
public class RunespanControler extends Controller {
		...
		if (largeIsland || true)
			...
			for (int runeId : plataform.runes) {
				if (runeId == ELEMENTAL_RUNE) {
					...
					player.getPackets().sendGameMessage("You need 1 " + ItemConfig.forID(runeId).getName() + " to cross this.");
					return true;
				}
			}
			...
			for (int runeId : runes) {
				if (builder.length() != 0)
					builder.append(" & ");
				builder.append("1 " + ItemConfig.forID(runeId).getName());
	...
	public void handleWizzard(Player player) {
		startTime = Utils.currentTimeMillis();
		yellowWizard = new YellowWizard(player.transform(1, 0, 0), this);//new WorldTile(RANDOM_LOCATIONS[Utils.random(RANDOM_LOCATIONS.length)]), this);
	//	if (!player.getHintIconsManager().isEmpty())
			//player.getHintIconsManager().removeUnsavedHintIcon();
			
			player.getHintIconsManager().addHintIcon(yellowWizard, 0, -1, false);
			
		player.getPackets().sendGameMessage("<col=FF0000>You hear a wizard calling for help, find him and you may receive a reward.</col>");
	}
	...
	//	player.getPackets().sendIComponentText(1274, 5, Integer.toString(player.getRuneSpanPoints()));
		player.getPackets().sendCSVarInteger(1909, getRuneSpanPoints());
	...
	public void refreshInventoryPoints() {
		...
		player.getPackets().sendCSVarInteger(1916, getRuneSpanPoints());
	}
	...
	public boolean processNPCClick1(NPC npc) {
		if (npc == yellowWizard) {
			YellowWizard.giveReward(player);
			yellowWizard.finish();
			return false;
		}
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (object.getId() == 70508) {
			...
			player.getPackets().sendCSVarInteger(1917, 3);
			return false;
		} else if (object.getId() == 70511) {
			player.useStairs(16668, new WorldTile(4107, 6043, 1), 4, 5);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					player.setNextAnimation(new Animation(16674));

...
public class SawmillController extends Controller {
	...
	public void sendInterfaces() {
		if (getJob() != null)
			player.getInterfaceManager().setOverlay(766, true);
	}
	...
	public void finishJob() {
		...
		player.incrementSawmillJobs();
		if (Utils.random(job.jobDetails == Jobs.QUICK ? 6 : 3) == 0)
			dropSet(player);
		setJob(null);
		player.getInterfaceManager().removeOverlay(true);
	}
	...
	public static final int[] PIECES = {10933, 10939, 10940, 10941};
	...
	private void dropSet(Player player) {
		...
		player.getCollectionLog().add(CategoryType.MINIGAMES, "Sawmill", new Item(piece));
		World.sendNews(player, player.getDisplayName() + " has received <col=ffff00>" + ItemConfig.forID(piece).getName() + "<col=ff8c38> from <col=cc33ff>sawmill<col=ff8c38>!", 1);
	}
	...
	public boolean isPlanksFull() {
		return planks >= 100;//Math.min(150, 100 + player.getDonator()*5);
	}

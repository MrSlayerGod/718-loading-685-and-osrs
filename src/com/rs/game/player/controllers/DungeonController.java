...
import com.rs.game.player.dialogues.impl.dungeoneering.DestroyCurrentStone;
...
public class DungeonController extends Controller {
	...
	public void dropGateStone(boolean dead) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					int overload = player.getOverloadDelay();
					int prayerRenewal = player.getPrayerRenewalDelay();
					player.reset();
					player.setOverloadDelay(overload);
			...
			if (nextBloodHit % 25 == 0) {
				...
				if (npcsIndexes != null) {
					for (int npcIndex : npcsIndexes) {
						...
						if (npc == null || !npc.isUnderCombat() || !Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), player.getX(), player.getY(), player.getSize(), 0))
							continue;
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		case "anvil":
			for (int index = 0; index < Smithing.BARS[1].length; index++) {
				if (player.getInventory().containsItem(Smithing.BARS[1][index], 1)) {
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		...
		case "altar":
			...
			if (bone != null) {
				player.getActionManager().setAction(new BoneOffering(object, bone, 0)); //250% xp, if you want 350% use your own house
				return true;
			}
			return true;
		case "farming patch":
	...
	public void openDoor(WorldObject object) {
		...
		if (dungeon.enterRoom(player, roomReference.getX() + Utils.DOOR_ROTATION_DIR_X[object.getRotation()], roomReference.getY() + Utils.DOOR_ROTATION_DIR_Y[object.getRotation()]))
			;
		hideBar();
	}
		...
		if (dungeon.isAtRewardsScreen()) {
			...
			if (interfaceId == 950 && componentId == 24) {
				...
				if (componentId == 2)
					...
					if ((d != null && d instanceof DestroyCurrentStone) || packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET) {
						if (d != null)
	...
	private void stoneTeleport(boolean group) {
		...
		if (player.isCantWalk() || (group && dungeon.getParty().getLeaderPlayer().getControlerManager().getControler() instanceof Kalaboss))
			return;
		...
		Magic.sendTeleportSpell(player, 13288, 13285, 2516, 2517, group ? 64 : 32, 0, tile == null ? player : tile, 2, false, Magic.MAGIC_TELEPORT,  group ? DungeonConstants.GROUP_GATESTONE_RUNES : new int[] {});
		player.resetReceivedHits();
		player.unlock();
		player.setCantWalk(true);
	...
	private void portalGroupStoneTeleport() {
		...
		Magic.sendTeleportSpell(player, 14279, 13285, 2518, 2517, 1, 0, tile == null ? player : tile, 1, false, Magic.OBJECT_TELEPORT);
	}

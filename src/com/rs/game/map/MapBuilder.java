...
public final class MapBuilder {
	...
	//	int count = 0;
		...
		while (true) {
			int map = MapBuilder.findEmptyRegionHash(8, 8);
			if (map == -1)
				break;
			if (maps.contains(map)) {
				System.out.println("repeated: "+map);
				break;
			}
			maps.add(map);
	...
	private static final int MAX_REGION_X = 127; //127; client limits x to 127 for some reason
	...
	public static int findEmptyRegionHash(int widthChunks, int heightChunks) {
		...
		Logger.log(MapBuilder.class, "Failed to find map :"+widthChunks+", "+heightChunks);
		return -1;
	...
	public static final void copyMap(int fromRegionX, int fromRegionY, int toRegionX, int toRegionY, int widthRegions, int heightRegions, int[] fromPlanes, int[] toPlanes) {
		copyMap(fromRegionX, fromRegionY, toRegionX, toRegionY, widthRegions, heightRegions, 0, fromPlanes, toPlanes);
	}
	
	public static final void copyMap(int fromRegionX, int fromRegionY, int toRegionX, int toRegionY, int widthRegions, int heightRegions, int rotation, int[] fromPlanes, int[] toPlanes) {
		...
		for (int xOffset = 0; xOffset < widthRegions; xOffset++) {
			for (int yOffset = 0; yOffset < heightRegions; yOffset++) {
				...
				int[] rotatedOffset = DynamicRegion.translate(xOffset, yOffset, widthRegions, heightRegions, rotation);
				int toThisRegionX = toRegionX + rotatedOffset[0];
				int toThisRegionY = toRegionY + rotatedOffset[1];
	...
	public static final void copy3RatioSquare(int fromRegionX, int fromRegionY, int toRegionX, int toRegionY, int rotation, int... planes) {
		for (int i : planes) { //plane 1 and 2
			if (rotation == 0) {
				copyChunk(fromRegionX, fromRegionY, i, toRegionX, toRegionY, i, rotation);
				copyChunk(fromRegionX + 1, fromRegionY, i, toRegionX + 1, toRegionY, i, rotation);
				copyChunk(fromRegionX + 2, fromRegionY, i, toRegionX + 2, toRegionY, i, rotation);
				copyChunk(fromRegionX + 3, fromRegionY, i, toRegionX + 3, toRegionY, i, rotation);
				
				copyChunk(fromRegionX, fromRegionY + 1, i, toRegionX, toRegionY + 1, i, rotation);
				copyChunk(fromRegionX + 1, fromRegionY + 1, i, toRegionX + 1, toRegionY + 1, i, rotation);
				copyChunk(fromRegionX + 1, fromRegionY + 2, i, toRegionX + 2, toRegionY + 1, i, rotation);
				copyChunk(fromRegionX + 1, fromRegionY + 3, i, toRegionX + 3, toRegionY + 1, i, rotation);
			} else if (rotation == 1) {
				copyChunk(fromRegionX, fromRegionY, i, toRegionX, toRegionY + 1, i, rotation);
				copyChunk(fromRegionX + 1, fromRegionY, i, toRegionX, toRegionY, i, rotation);
				copyChunk(fromRegionX, fromRegionY + 1, i, toRegionX + 1, toRegionY + 1, i, rotation);
	...
	public static void destroyRegion(int regionId) {
		Region region = World.getRegions().get(regionId);
		if (!(region instanceof DynamicRegion) && regionId != 9520)  //shoudnt happen
			return;
		if (region != null) {

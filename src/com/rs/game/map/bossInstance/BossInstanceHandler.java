...
import com.rs.game.map.bossInstance.impl.ArmadylInstance;
import com.rs.game.map.bossInstance.impl.BandosInstance;
...
import com.rs.game.map.bossInstance.impl.KrakenInstance;
import com.rs.game.map.bossInstance.impl.SaradominInstance;
import com.rs.game.map.bossInstance.impl.ZamorakInstance;
...
public class BossInstanceHandler {
	...
	public static enum Boss {
		...
		, Corporeal_Beast(CorporealBeastInstance.class, 200000, 200, false, true, new WorldTile(2970, 4384, 2), new WorldTile(2974, 4384, 2), null, "CorporealBeastInstanceController", 617)
		, Kraken(KrakenInstance.class, 25000, 200, false, true, new WorldTile(2280, 10016, 0), new WorldTile(2280, 10022, 0), null, "KrakenInstanceController", -1621)
		, General_Graardor(BandosInstance.class, 200000, 200, true, true, new WorldTile(2862, 5357, 0), new WorldTile(2863, 5357, 0), null, "GodwarsInstanceController", 495)
		, Kree_arra(ArmadylInstance.class, 200000, 200, true, true, new WorldTile(2835, 5294, 0), new WorldTile(2835, 5295, 0), null, "GodwarsInstanceController", 493)
		, Kril_Tsutsaroth(ZamorakInstance.class, 200000, 200, true, true, new WorldTile(2925, 5333, 0), new WorldTile(2925, 5332, 0), null, "GodwarsInstanceController", 490)
		, Commander_Zilyana(SaradominInstance.class, 200000, 200, false, true, new WorldTile(2923, 5257, 0), new WorldTile(2923, 5256, 0), null, "GodwarsInstanceController", 491)
	...
	public static BossInstance joinInstance(Player player, Boss boss, String key, boolean login) {
		synchronized (LOCK) {
			...
			if (!key.equals("") && !player.getUsername().equals(key)) {
				...
				if (owner == null || owner.getFriendsIgnores().getPmStatus() == FriendsIgnores.PM_STATUS_OFFLINE) {
					player.getPackets().sendGameMessage("That player is offline, or has privacy mode enabled.");
					return null;
				}

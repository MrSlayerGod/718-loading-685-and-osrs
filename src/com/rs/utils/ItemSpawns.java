...
public final class ItemSpawns {
	...
	public static final void init() {
		if (!new File("data/item/packedSpawns").exists())
			packItemSpawns();
	}

	private static final void packItemSpawns() {
		Logger.log("ItemSpawns", "Packing item spawns...");
		if (!new File("data/item/packedSpawns").mkdir())
			throw new RuntimeException("Couldn't create packedSpawns directory.");
		try {
			BufferedReader in = new BufferedReader(new FileReader("data/item/unpackedSpawnsList.txt"));
			while (true) {
	...
	public static final void loadItemSpawns(int regionId) {
		File file = new File("data/item/packedSpawns/" + regionId + ".is");
		if (!file.exists())
			return;
		try {
			...
			while (buffer.hasRemaining()) {
				...
				World.addGroundItemForever(new Item(itemId, x == 1534 && y == 3591 ? 1000 : itemId == 995 ? (Utils.random(999)+1) : ItemConfig.forID(itemId).isStackable() ? Settings.getDropQuantityRate() : 1), new WorldTile(x, y, plane));
			}
			channel.close();
			in.close();
	...
	private static final void addItemSpawn(int itemId, int regionId, WorldTile tile) {
		try {
			DataOutputStream out = new DataOutputStream(new FileOutputStream("data/item/packedSpawns/" + regionId + ".is", true));

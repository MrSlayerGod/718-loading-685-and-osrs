...
public class RuneWikiExaminesDumper {
	...
	public static final void main(String[] args) throws IOException {
		...
		for (int itemId = 25058; itemId < Utils.getItemDefinitionsSize(); itemId++) {
			if (!ItemConfig.forID(itemId).isNoted())
				if (dumpItem(itemId))
					System.out.println("DUMPED ITEM : " + itemId);
				else
					System.out.println("FAILED ITEM: " + itemId + ", " + ItemConfig.forID(itemId).getName());
		}
	}

	public static boolean dumpItem(int itemId) {
		String pageName = ItemConfig.forID(itemId).getName();

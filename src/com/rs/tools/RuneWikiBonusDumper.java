...
public class RuneWikiBonusDumper {
	...
	public static final void main(String[] args) throws IOException {
		...
		for (int itemId = 22067; itemId < Utils.getItemDefinitionsSize(); itemId++) {
			if (ItemConfig.forID(itemId).isWearItem() && !ItemConfig.forID(itemId).isNoted())
				if (dumpItem(itemId))
					System.out.println("Dumped ITEM: " + itemId + ", " + ItemConfig.forID(itemId).getName());
		}
	}
	...
	public static boolean dumpItem(int itemId) {
		...
		if (file.exists())
			return false;
		String pageName = ItemConfig.forID(itemId).getName().replace(" (black)", "").replace(" (white)", "").replace(" (yellow)", "").replace(" (red)", "");

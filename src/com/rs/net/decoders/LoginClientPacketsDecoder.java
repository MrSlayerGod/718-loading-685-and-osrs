...
public class LoginClientPacketsDecoder {
	...
	public static void decodeIncomingPacket(InputStream stream) {
		...
		case LoginProtocol.PACKET_PINGPONG:
			...
			LoginClientChannelManager.sendReliablePacket(LoginChannelsPacketEncoder.encodePingPong(number++).trim());
			break;
		...
		case LoginProtocol.PACKET_LOGINRSP:
			...
			if (status == 2) {
				PlayerHandlerThread.handleLoginResponse(sessionid, sessionuser, status, file_length, stream.readUnsignedByte()
						, stream.readUnsignedByte() //game mode
						, stream.readUnsignedByte() == 1 //master login
						, stream.readUnsignedByte() //donator
						, stream.readUnsignedByte() == 1 //support
						, stream.readUnsignedByte() == 1 //gfx designer
								, stream.readUnsignedByte() == 1 //gfx youtuber
						, stream.readUnsignedByte() //message icon
						, stream.readUnsignedByte() == 1 //muted 
						, stream.readLong(), stream.readNullString(), stream.readNullString());
			} else {
				PlayerHandlerThread.handleLoginResponse(sessionid, sessionuser, status, file_length, 0, 0, false, 0, false, false, false, 0, false, 0, null, null);
			}
			break;
		...
		case LoginProtocol.PACKET_UPDATEPLAYERINTVAR:
			...
			if (player != null) {
				...
				case LoginProtocol.VAR_TYPE_RIGHTS:
					...
					break;
				case LoginProtocol.VAR_TYPE_RANKS: //TODO
					int id = var & 0xFFFF;
		...
		case LoginProtocol.PACKET_PLAYER_UPDATEFRIENDSINFO:
			...
			if (player != null && player.hasStarted()) {
				...
				while (stream.readByte() == 1) {
					...
					boolean bot = false;
					for (Player p2 : World.BOTS)
						if (p2.getDisplayName().equalsIgnoreCase(displayName))
							bot = true;
					if (bot) {
						world = 1;
						worldName = "World 1";
		...
		case LoginProtocol.PACKET_PLAYER_SENTPRIVATE:
			...
			if (isQc) {
				...
				if (player == null)
					player = World.getLobbyPlayer(username);
				if (player != null && player.hasStarted())
					player.getPackets().sendPrivateMessageNew(name, qcFileId, qcData);
			} else {
				message = stream.readString();
				player = World.getPlayer(username);
				if (player == null)
				...
				if (player != null && player.hasStarted()) {
					...
					player.getPackets().sendPrivateMessageNew(name, message);
				}
			}
			break;
		case LoginProtocol.PACKET_PLAYER_RECVPRIVATE:
			...
			if (isQc) {
				...
				if (player == null)
					player = World.getLobbyPlayer(username);
				if (player != null && player.hasStarted())
					player.getPackets().receivePrivateMessageNew(name, messageUid, iconId, qcFileId, qcData);
			} else {
				message = stream.readString();
				player = World.getPlayer(username);
				if (player == null)
				...
				if (player != null && player.hasStarted()) {
					if (player.isFilteringProfanity())
						message = Censor.getFilteredMessage(message);
					message = Utils.fixChatMessage(message);
					player.getPackets().receivePrivateMessageNew(name, messageUid, iconId, message);
				}
			}
			break;
		...
		case LoginProtocol.PACKET_PLAYER_FRIENDSCHATUPDATEINFO:
			...
			if (player != null && player.hasStarted() && stream.getRemaining() > 0) {
				...
				List<Player> bots = !ownerUsername.contains(World.BOT_FC) ? null : new ArrayList<Player>(World.BOTS);
				if (bots != null) {
					for (Player bot : bots.toArray(new Player[bots.size()])) {
						if (!FriendsChat.contains(ownerUsername, bot))
							bots.remove(bot);
					}
					while (!bots.isEmpty() && bots.size() + membersCount > 100)
						bots.remove(0);
				}
				
				OutputStream packet = player.getPackets().startFriendsChatChannel(ownerDisplayname, name, kickReq, membersCount + (bots != null ? bots.size() : 0));
				while (stream.readByte() == 1) {
					String displayName = stream.readString();
				...
				if (bots != null)
					for (Player bot : bots) 
						player.getPackets().appendFriendsChatMember(packet, bot.getDisplayName(), 1, -1, "World 1");
